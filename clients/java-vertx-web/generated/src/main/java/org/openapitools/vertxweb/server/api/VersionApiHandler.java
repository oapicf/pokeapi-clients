...
public class VersionApiHandler {

    private static final Logger logger = LoggerFactory.getLogger(VersionApiHandler.class);

    private final VersionApi api;

    public VersionApiHandler(VersionApi api) {
        this.api = api;
    }

    @Deprecated
    public VersionApiHandler() {
        this(new VersionApiImpl());
    }

    public void mount(RouterBuilder builder) {
        builder.operation("versionList").handler(this::versionList);
        builder.operation("versionRead").handler(this::versionRead);
    }

    private void versionList(RoutingContext routingContext) {
    ...
    private void versionRead(RoutingContext routingContext) {
        logger.info("versionRead()");
