...
Experience::Experience(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string Experience::toJsonString(bool prettyJson /* = false */) const
{
...
void Experience::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value Experience::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void Experience::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for Experience");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object Experience::toJsonObject_internal() const
{
    boost::json::object object;
        object["level"] = JsonValueConverter<int32_t>::toJsonValue(getLevel());
        object["experience"] = JsonValueConverter<int32_t>::toJsonValue(getExperience());
    return object;
}

void Experience::fromJsonObject_internal(boost::json::object const& object)
{
    {
    ...
    {
        const auto ExperienceIt = object.find("experience");
        if (ExperienceIt != object.end()) {
            setExperience(JsonValueConverter<int32_t>::fromJsonValue(ExperienceIt->value()));
        }
    }
}
...
void Experience::setExperience(int32_t value)
{
    ...
    m_Experience = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<Experience>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<Experience>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<Experience>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<Experience>>>::fromJsonValue(boost::json::parse(json));
}
