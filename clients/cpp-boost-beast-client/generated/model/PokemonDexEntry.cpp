...
PokemonDexEntry::PokemonDexEntry(boost::json::value const& value)
{
    fromJsonValue(value);
}
...
boost::json::value PokemonDexEntry::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void PokemonDexEntry::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for PokemonDexEntry");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object PokemonDexEntry::toJsonObject_internal() const
{
    boost::json::object object;
        object["entry_number"] = JsonValueConverter<int32_t>::toJsonValue(getEntryNumber());
        object["pokedex"] = JsonValueConverter<std::shared_ptr<PokedexSummary>>::toJsonValue(getPokedex());
    return object;
}

void PokemonDexEntry::fromJsonObject_internal(boost::json::object const& object)
{
    {
    ...
    {
        const auto PokedexIt = object.find("pokedex");
        if (PokedexIt != object.end()) {
            setPokedex(JsonValueConverter<std::shared_ptr<PokedexSummary>>::fromJsonValue(PokedexIt->value()));
        }
    }
}
...
void PokemonDexEntry::setPokedex(std::shared_ptr<PokedexSummary> value)
{
    
    m_Pokedex = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<PokemonDexEntry>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<PokemonDexEntry>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<PokemonDexEntry>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<PokemonDexEntry>>>::fromJsonValue(boost::json::parse(json));
}
