...
TypeGameIndex::TypeGameIndex(boost::json::value const& value)
{
    fromJsonValue(value);
}

std::string TypeGameIndex::toJsonString(bool prettyJson /* = false */) const
{
...
void TypeGameIndex::fromJsonString(std::string const& jsonString)
{
    fromJsonValue(boost::json::parse(jsonString));
}

boost::json::value TypeGameIndex::toJsonValue() const
{
    return boost::json::value(toJsonObject_internal());
}

void TypeGameIndex::fromJsonValue(boost::json::value const& value)
{
    if (!value.is_object()) {
        throw std::invalid_argument("Expected a JSON object for TypeGameIndex");
    }
    fromJsonObject_internal(value.as_object());
}

boost::json::object TypeGameIndex::toJsonObject_internal() const
{
...
void TypeGameIndex::fromJsonObject_internal(boost::json::object const& object)
{
    {
...
void TypeGameIndex::setGameIndex(int32_t value)
{
    
    m_Game_index = std::move(value);
}
std::shared_ptr<GenerationSummary> TypeGameIndex::getGeneration() const
{
    return m_Generation;
}

void TypeGameIndex::setGeneration(std::shared_ptr<GenerationSummary> value)
{
    
    m_Generation = std::move(value);
}

std::string createJsonStringFromModelVector(const std::vector<std::shared_ptr<TypeGameIndex>>& data)
{
    return boost::json::serialize(JsonValueConverter<std::vector<std::shared_ptr<TypeGameIndex>>>::toJsonValue(data));
}

void createModelVectorFromJsonString(std::vector<std::shared_ptr<TypeGameIndex>>& vec, const std::string& json)
{
    vec = JsonValueConverter<std::vector<std::shared_ptr<TypeGameIndex>>>::fromJsonValue(boost::json::parse(json));
}
