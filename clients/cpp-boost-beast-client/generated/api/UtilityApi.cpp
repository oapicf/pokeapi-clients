...
UtilityApiException::UtilityApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status UtilityApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<PaginatedLanguageSummaryList>
UtilityApi::languageList(
    ...
    std::string path = m_context + "/api/v2/language";
    ...
    std::shared_ptr<PaginatedLanguageSummaryList> deserializedResponse = std::make_shared<PaginatedLanguageSummaryList>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<PaginatedLanguageSummaryList>>::deserialize(
    ...
    throw UtilityApiException(statusCode, "Unexpected HTTP status code");
}
std::shared_ptr<LanguageDetail>
UtilityApi::languageRetrieve(
    const std::string& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/api/v2/language/{id}";
    std::map<std::string, std::string> headers;
    // path params
    ...
    std::shared_ptr<LanguageDetail> deserializedResponse = std::make_shared<LanguageDetail>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<LanguageDetail>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string UtilityApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void UtilityApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void UtilityApi::handleStdException(const std::exception&) {
    throw;
}

void UtilityApi::handleUncaughtException() {
    throw;
}
