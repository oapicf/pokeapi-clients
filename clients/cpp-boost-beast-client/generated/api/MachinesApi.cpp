...
MachinesApiException::MachinesApiException(boost::beast::http::status statusCode, std::string what)
  : m_status(statusCode),
    m_what(what)
{
}

boost::beast::http::status MachinesApiException::getStatus() const
{
    return m_status;
}
...
std::shared_ptr<PaginatedMachineSummaryList>
MachinesApi::machineList(
    ...
    std::string path = m_context + "/api/v2/machine";
    ...
    std::shared_ptr<PaginatedMachineSummaryList> deserializedResponse = std::make_shared<PaginatedMachineSummaryList>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<PaginatedMachineSummaryList>>::deserialize(
    ...
    throw MachinesApiException(statusCode, "Unexpected HTTP status code");
}
std::shared_ptr<MachineDetail>
MachinesApi::machineRetrieve(
    const std::string& id) {
    std::string serializedRequestBody;
    std::string path = m_context + "/api/v2/machine/{id}";
    std::map<std::string, std::string> headers;
    // path params
    ...
    std::shared_ptr<MachineDetail> deserializedResponse = std::make_shared<MachineDetail>();
    if (statusCode == boost::beast::http::status(200)) {
        ResponseBodyDeserializer<std::shared_ptr<MachineDetail>>::deserialize(
            deserializedResponse,
            responseBody,
            responseContentType,
            false);
...
std::string MachinesApi::base64encode(const std::string& str) {
    return base64encodeImpl(str);
}

void MachinesApi::setPreferredMediaTypeHeader(std::map<std::string, std::string>& headers, const std::string& headerName, const std::vector<std::string>& contentTypes) {
    const std::string contentType = selectPreferredContentType(contentTypes);
    headers[headerName] = contentType;
}

void MachinesApi::handleStdException(const std::exception&) {
    throw;
}

void MachinesApi::handleUncaughtException() {
    throw;
}
