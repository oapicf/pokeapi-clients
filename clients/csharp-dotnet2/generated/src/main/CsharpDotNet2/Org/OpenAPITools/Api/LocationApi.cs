...
namespace Org.OpenAPITools.Api
{
    ...
    public class LocationApi : ILocationApi
    {
        ...
        /// <param name="offset"></param>
        /// <returns>string</returns>
        public string LocationList (int? limit, int? offset)
        {
            

            var path = "/api/v2/location/";
 ...
 if (offset != null) queryParams.Add("offset", ApiClient.ParameterToString(offset)); // query parameter
        ...
        /// <param name="id"></param>
        /// <returns>string</returns>
        public string LocationRead (int? id)
        {
            
            // verify the required parameter 'id' is set
            if (id == null) throw new ApiException(400, "Missing required parameter 'id' when calling LocationRead");
            

            var path = "/api/v2/location/{id}/";
            path = path.Replace("{format}", "json");
            ...
            if (((int)response.StatusCode) >= 400)
                throw new ApiException ((int)response.StatusCode, "Error calling LocationRead: " + response.Content, response.Content);
            else if (((int)response.StatusCode) == 0)
                throw new ApiException ((int)response.StatusCode, "Error calling LocationRead: " + response.ErrorMessage, response.ErrorMessage);
