...
void OAILanguageApi::initializeServerConfigs() {
    ...
    _serverConfigs.insert("languageRead", defaultConf);
    _serverIndices.insert("languageRead", 0);
}
...
void OAILanguageApi::languageList(const ::OpenAPI::OptionalParam<qint32> &limit, const ::OpenAPI::OptionalParam<qint32> &offset) {
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAILanguageApi::languageListCallback);
...
void OAILanguageApi::languageListCallback(OAIHttpRequestWorker *worker) {
...
void OAILanguageApi::languageRead(const qint32 &id) {
    QString fullPath = QString(_serverConfigs["languageRead"][_serverIndices.value("languageRead")].URL()+"/api/v2/language/{id}/");
    ...
    connect(worker, &OAIHttpRequestWorker::on_execution_finished, this, &OAILanguageApi::languageReadCallback);
    connect(this, &OAILanguageApi::abortRequestsSignal, worker, &QObject::deleteLater);
    connect(worker, &QObject::destroyed, this, [this]() {
        if (findChildren<OAIHttpRequestWorker*>().count() == 0) {
            Q_EMIT allPendingRequestsCompleted();
...
void OAILanguageApi::languageReadCallback(OAIHttpRequestWorker *worker) {
    QString error_str = worker->error_str;
    QNetworkReply::NetworkError error_type = worker->error_type;
    ...
    if (worker->error_type == QNetworkReply::NoError) {
        Q_EMIT languageReadSignal(output);
        Q_EMIT languageReadSignalFull(worker, output);
    } else {
...
#pragma GCC diagnostic ignored "-Wdeprecated-declarations"
        ...
        Q_EMIT languageReadSignalE(output, error_type, error_str);
        Q_EMIT languageReadSignalEFull(worker, error_type, error_str);
...
#pragma GCC diagnostic pop
        ...
        Q_EMIT languageReadSignalError(output, error_type, error_str);
        Q_EMIT languageReadSignalErrorFull(worker, error_type, error_str);
    }
}
