...
- `GET` to `/api/v2/pokemon/:id/`, OperationId: `pokemon_read`:
.
...
allowed_methods(Req, #state{operation_id = 'pokemon_list'} = State) ->
    {[<<"GET">>], Req, State};
allowed_methods(Req, #state{operation_id = 'pokemon_read'} = State) ->
    {[<<"GET">>], Req, State};
...
content_types_accepted(Req, #state{operation_id = 'pokemon_list'} = State) ->
    {[], Req, State};
content_types_accepted(Req, #state{operation_id = 'pokemon_read'} = State) ->
    {[], Req, State};
...
valid_content_headers(Req, #state{operation_id = 'pokemon_list'} = State) ->
    {true, Req, State};
valid_content_headers(Req, #state{operation_id = 'pokemon_read'} = State) ->
    {true, Req, State};
...
content_types_provided(Req, #state{operation_id = 'pokemon_read'} = State) ->
    {[
      {<<"text/plain">>, handle_type_provided}
     ], Req, State};
