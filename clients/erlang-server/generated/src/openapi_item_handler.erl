...
- `GET` to `/api/v2/item/:id/`, OperationId: `item_read`:
.
...
allowed_methods(Req, #state{operation_id = 'item_list'} = State) ->
    {[<<"GET">>], Req, State};
allowed_methods(Req, #state{operation_id = 'item_read'} = State) ->
    {[<<"GET">>], Req, State};
...
content_types_accepted(Req, #state{operation_id = 'item_list'} = State) ->
    {[], Req, State};
content_types_accepted(Req, #state{operation_id = 'item_read'} = State) ->
    {[], Req, State};
...
valid_content_headers(Req, #state{operation_id = 'item_list'} = State) ->
    {true, Req, State};
valid_content_headers(Req, #state{operation_id = 'item_read'} = State) ->
    {true, Req, State};
...
content_types_provided(Req, #state{operation_id = 'item_read'} = State) ->
    {[
      {<<"text/plain">>, handle_type_provided}
     ], Req, State};
...
handle_type_accepted(Req, #state{operation_id = OperationID,
    ...
    {Res, Req1, Context1} = Handler(item, OperationID, Req, Context),
