...
- `GET` to `/api/v2/location/:id/`, OperationId: `location_read`:
.
...
allowed_methods(Req, #state{operation_id = 'location_list'} = State) ->
    {[<<"GET">>], Req, State};
allowed_methods(Req, #state{operation_id = 'location_read'} = State) ->
    {[<<"GET">>], Req, State};
...
content_types_accepted(Req, #state{operation_id = 'location_list'} = State) ->
    {[], Req, State};
content_types_accepted(Req, #state{operation_id = 'location_read'} = State) ->
    {[], Req, State};
...
valid_content_headers(Req, #state{operation_id = 'location_list'} = State) ->
    {true, Req, State};
valid_content_headers(Req, #state{operation_id = 'location_read'} = State) ->
    {true, Req, State};
...
content_types_provided(Req, #state{operation_id = 'location_read'} = State) ->
    {[
      {<<"text/plain">>, handle_type_provided}
     ], Req, State};
