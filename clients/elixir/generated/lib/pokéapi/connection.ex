...
defmodule PokéAPI.Connection do
  @moduledoc """
  Handle Tesla connections for PokéAPI.

  Additional middleware can be set in the compile-time or runtime configuration:

      config :tesla, PokéAPI.Connection,
  ...
  @doc """
  Configure a PokéAPI client.

  ### Parameters

  - `options`: an optional keyword list of PokéAPI.Connection.options.
  ...
  def middleware(options \\ []) do
    base_url =
      Keyword.get(
        options,
        :base_url,
        Application.get_env(:"pokéapi", :base_url, @default_base_url)
      )
    ...
    user_agent =
      Keyword.get(
        ...
        Keyword.get(
          tesla_options,
          :user_agent,
          "openapi-generator - PokéAPI 1.0.0 - elixir"
        )
      )
