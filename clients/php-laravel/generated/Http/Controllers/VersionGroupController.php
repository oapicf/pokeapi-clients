...
class VersionGroupController extends Controller
{
    ...
    public function __construct(
        private readonly VersionGroupApiInterface $api,
    ...
    public function versionGroupList(Request $request): JsonResponse
    {
        ...
        try {
            $apiResult = $this->api->versionGroupList($limit, $offset);
     ...
     *
     */
    public function versionGroupRead(Request $request, int $id): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
        ...
        try {
            $apiResult = $this->api->versionGroupRead($id);
        } catch (\Exception $exception) {
            // This shouldn't happen
            return response()->json(['error' => $exception->getMessage()], 500);
        }
