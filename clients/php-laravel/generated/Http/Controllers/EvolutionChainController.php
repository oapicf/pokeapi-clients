...
class EvolutionChainController extends Controller
{
    ...
    public function __construct(
        private readonly EvolutionChainApiInterface $api,
    ...
    public function evolutionChainList(Request $request): JsonResponse
    {
        ...
        try {
            $apiResult = $this->api->evolutionChainList($limit, $offset);
     ...
     *
     */
    public function evolutionChainRead(Request $request, int $id): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
        ...
        try {
            $apiResult = $this->api->evolutionChainRead($id);
        } catch (\Exception $exception) {
            // This shouldn't happen
            return response()->json(['error' => $exception->getMessage()], 500);
        }
