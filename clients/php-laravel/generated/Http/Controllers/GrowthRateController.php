...
class GrowthRateController extends Controller
{
    ...
    public function __construct(
        private readonly GrowthRateApiInterface $api,
    ...
    public function growthRateList(Request $request): JsonResponse
    {
        ...
        try {
            $apiResult = $this->api->growthRateList($limit, $offset);
     ...
     *
     */
    public function growthRateRead(Request $request, int $id): JsonResponse
    {
        $validator = Validator::make(
            array_merge(
                [
        ...
        try {
            $apiResult = $this->api->growthRateRead($id);
        } catch (\Exception $exception) {
            // This shouldn't happen
            return response()->json(['error' => $exception->getMessage()], 500);
        }
