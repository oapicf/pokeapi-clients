...
Protected Class EncountersApi
	...
	#tag Method, Flags = &h0
		Sub PokemonEncountersRetrieve(, pokemonId As String)
		  // Operation pokemon_encounters_retrieve
		  // Get pokemon encounter
		  // - 
		  // - parameter pokemonId: (path)  
		  //
		  // Invokes EncountersApiCallbackHandler.PokemonEncountersRetrieveCallback(PokemonEncountersRetrieve200ResponseInner) on completion. 
		  //
		  ...
		  AddHandler localVarHTTPSocket.AuthenticationRequired, addressof Me.AuthenticationRequiredIf me.ApiKeycookieAuth = "" Then Raise New OpenAPIClient.OpenAPIClientException(kErrorCannotAuthenticate, "API key is unset. Please assign a value to `EncountersApi.ApiKeycookieAuth` before invoking `EncountersApi.PokemonEncountersRetrieve()`.")
		  
		  
		  


		  Dim localVarPath As String = "/api/v2/pokemon/{pokemon_id}/encounters"
		  
		  Dim localVarPathStringpokemonId As String = pokemonId
		  
		  localVarPath = localVarPath.ReplaceAllB("{pokemon_id}", localVarPathStringpokemonId)
		  
		  
		  AddHandler localVarHTTPSocket.PageReceived, addressof me.PokemonEncountersRetrieve_handler
		  AddHandler localVarHTTPSocket.Error, addressof Me.PokemonEncountersRetrieve_error
	...
	#tag Method, Flags = &h21
		Private Function PokemonEncountersRetrievePrivateFuncDeserializeResponse(HTTPStatus As Integer, Headers As InternetHeaders, error As OpenAPIClient.OpenAPIClientException, Content As String, outData() As OpenAPIClient.Models.PokemonEncountersRetrieve200ResponseInner) As Boolean
		  Dim contentType As String = Headers.Value("Content-Type")
		  Dim contentEncoding As TextEncoding = OpenAPIClient.EncodingFromContentType(contentType)
		  Content = DefineEncoding(Content, contentEncoding)
	...
	#tag Method, Flags = &h21
		Private Sub PokemonEncountersRetrieve_error(sender As HTTPSecureSocket, Code As Integer)
		  If sender <> nil Then sender.Close()

		  Dim error As New OpenAPIClient.OpenAPIClientException(Code)
		  Dim data() As OpenAPIClient.Models.PokemonEncountersRetrieve200ResponseInner
		  CallbackHandler.PokemonEncountersRetrieveCallback(error, data)
		End Sub
	#tag EndMethod

	#tag Method, Flags = &h21
		Private Sub PokemonEncountersRetrieve_handler(sender As HTTPSecureSocket, URL As String, HTTPStatus As Integer, Headers As InternetHeaders, Content As String)
		  #Pragma Unused URL
		  ...
		  Dim data() As OpenAPIClient.Models.PokemonEncountersRetrieve200ResponseInner
		  Call PokemonEncountersRetrievePrivateFuncDeserializeResponse(HTTPStatus, Headers, error, Content, data)
