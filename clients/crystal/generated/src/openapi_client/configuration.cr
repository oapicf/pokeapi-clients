...
module OpenAPIClient
  class Configuration
    ...
    def apply_auth!(headers : HTTP::Headers, params : Hash(String, String | Array(String)), auth_names : Array(String)) : Nil
      if auth_names.includes?("basicAuth")
        if @username || @password
          headers["Authorization"] = basic_auth_token
        end
      end
      if auth_names.includes?("cookieAuth")
        value = api_key_with_prefix(:"sessionid")
        unless value.empty?
          headers["Cookie"] = [headers["Cookie"]?, "sessionid=#{value}"].compact.join("; ")
        end
      end
    end
