...
(defn-spec evolution-chain-list-with-http-info any?
  "List evolution chains
  Evolution chains are essentially family trees. They start with the lowest stage within a family and detail evolution conditions for each as well as Pokémon they can evolve into up through the hierarchy."
  ([] (evolution-chain-list-with-http-info nil))
  ([{:keys [limit offset q]} (s/map-of keyword? any?)]
   (call-api "/api/v2/evolution-chain/" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec evolution-chain-list paginated-evolution-chain-summary-list-spec
  "List evolution chains
  Evolution chains are essentially family trees. They start with the lowest stage within a family and detail evolution conditions for each as well as Pokémon they can evolve into up through the hierarchy."
  ([] (evolution-chain-list nil))
  ([optional-params any?]
   (let [res (:data (evolution-chain-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-evolution-chain-summary-list-spec res st/string-transformer)
        res))))


(defn-spec evolution-chain-retrieve-with-http-info any?
  "Get evolution chain
  Evolution chains are essentially family trees. They start with the lowest stage within a family and detail evolution conditions for each as well as Pokémon they can evolve into up through the hierarchy."
  [id string?]
  (check-required-params id)
  (call-api "/api/v2/evolution-chain/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec evolution-chain-retrieve evolution-chain-detail-spec
  "Get evolution chain
  Evolution chains are essentially family trees. They start with the lowest stage within a family and detail evolution conditions for each as well as Pokémon they can evolve into up through the hierarchy."
  [id string?]
  (let [res (:data (evolution-chain-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode evolution-chain-detail-spec res st/string-transformer)
       res)))
...
(defn-spec evolution-trigger-list paginated-evolution-trigger-summary-list-spec
  "List evolution triggers
  Evolution triggers are the events and conditions that cause a Pokémon to evolve. Check out [Bulbapedia](http://bulbapedia.bulbagarden.net/wiki/Methods_of_evolution) for greater detail."
  ...
  ([optional-params any?]
   (let [res (:data (evolution-trigger-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-evolution-trigger-summary-list-spec res st/string-transformer)
        res))))


(defn-spec evolution-trigger-retrieve-with-http-info any?
...
(defn-spec evolution-trigger-retrieve evolution-trigger-detail-spec
  "Get evolution trigger
  Evolution triggers are the events and conditions that cause a Pokémon to evolve. Check out [Bulbapedia](http://bulbapedia.bulbagarden.net/wiki/Methods_of_evolution) for greater detail."
  [id string?]
  (let [res (:data (evolution-trigger-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode evolution-trigger-detail-spec res st/string-transformer)
       res)))
