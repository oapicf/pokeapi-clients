...
(defn-spec move-ailment-list-with-http-info any?
  "List move meta ailments
  Move Ailments are status conditions caused by moves used during battle. See [Bulbapedia](https://bulbapedia.bulbagarden.net/wiki/Status_condition) for greater detail."
  ([] (move-ailment-list-with-http-info nil))
  ([{:keys [limit offset q]} (s/map-of keyword? any?)]
   (call-api "/api/v2/move-ailment/" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec move-ailment-list paginated-move-meta-ailment-summary-list-spec
  "List move meta ailments
  Move Ailments are status conditions caused by moves used during battle. See [Bulbapedia](https://bulbapedia.bulbagarden.net/wiki/Status_condition) for greater detail."
  ([] (move-ailment-list nil))
  ([optional-params any?]
   (let [res (:data (move-ailment-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-move-meta-ailment-summary-list-spec res st/string-transformer)
        res))))


(defn-spec move-ailment-retrieve-with-http-info any?
  "Get move meta ailment
  Move Ailments are status conditions caused by moves used during battle. See [Bulbapedia](https://bulbapedia.bulbagarden.net/wiki/Status_condition) for greater detail."
  [id string?]
  (check-required-params id)
  (call-api "/api/v2/move-ailment/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec move-ailment-retrieve move-meta-ailment-detail-spec
  "Get move meta ailment
  Move Ailments are status conditions caused by moves used during battle. See [Bulbapedia](https://bulbapedia.bulbagarden.net/wiki/Status_condition) for greater detail."
  [id string?]
  (let [res (:data (move-ailment-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode move-meta-ailment-detail-spec res st/string-transformer)
       res)))
...
(defn-spec move-battle-style-list paginated-move-battle-style-summary-list-spec
  "List move battle styles
  Styles of moves when used in the Battle Palace. See [Bulbapedia](http://bulbapedia.bulbagarden.net/wiki/Battle_Frontier_(Generation_III)) for greater detail."
  ...
  ([optional-params any?]
   (let [res (:data (move-battle-style-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-move-battle-style-summary-list-spec res st/string-transformer)
        res))))


(defn-spec move-battle-style-retrieve-with-http-info any?
...
(defn-spec move-battle-style-retrieve move-battle-style-detail-spec
  "Get move battle style
  Styles of moves when used in the Battle Palace. See [Bulbapedia](http://bulbapedia.bulbagarden.net/wiki/Battle_Frontier_(Generation_III)) for greater detail."
  [id string?]
  (let [res (:data (move-battle-style-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode move-battle-style-detail-spec res st/string-transformer)
       res)))
...
(defn-spec move-category-list paginated-move-meta-category-summary-list-spec
  "List move meta categories
  Very general categories that loosely group move effects."
  ...
  ([optional-params any?]
   (let [res (:data (move-category-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-move-meta-category-summary-list-spec res st/string-transformer)
        res))))


(defn-spec move-category-retrieve-with-http-info any?
...
(defn-spec move-category-retrieve move-meta-category-detail-spec
  "Get move meta category
  Very general categories that loosely group move effects."
  [id string?]
  (let [res (:data (move-category-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode move-meta-category-detail-spec res st/string-transformer)
       res)))
...
(defn-spec move-learn-method-list paginated-move-learn-method-summary-list-spec
  "List move learn methods
  Methods by which Pokémon can learn moves."
  ...
  ([optional-params any?]
   (let [res (:data (move-learn-method-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-move-learn-method-summary-list-spec res st/string-transformer)
        res))))


(defn-spec move-learn-method-retrieve-with-http-info any?
...
(defn-spec move-learn-method-retrieve move-learn-method-detail-spec
  "Get move learn method
  Methods by which Pokémon can learn moves."
  [id string?]
  (let [res (:data (move-learn-method-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode move-learn-method-detail-spec res st/string-transformer)
       res)))


(defn-spec move-list-with-http-info any?
  "List moves
  Moves are the skills of Pokémon in battle. In battle, a Pokémon uses one move each turn. Some moves (including those learned by Hidden Machine) can be used outside of battle as well, usually for the purpose of removing obstacles or exploring new areas."
  ([] (move-list-with-http-info nil))
  ([{:keys [limit offset q]} (s/map-of keyword? any?)]
   (call-api "/api/v2/move/" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"limit" limit "offset" offset "q" q }
              :form-params   {}
...
(defn-spec move-list paginated-move-summary-list-spec
  "List moves
  Moves are the skills of Pokémon in battle. In battle, a Pokémon uses one move each turn. Some moves (including those learned by Hidden Machine) can be used outside of battle as well, usually for the purpose of removing obstacles or exploring new areas."
  ([] (move-list nil))
  ([optional-params any?]
   (let [res (:data (move-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-move-summary-list-spec res st/string-transformer)
        res))))


(defn-spec move-retrieve-with-http-info any?
  "Get move
  Moves are the skills of Pokémon in battle. In battle, a Pokémon uses one move each turn. Some moves (including those learned by Hidden Machine) can be used outside of battle as well, usually for the purpose of removing obstacles or exploring new areas."
  [id string?]
  (check-required-params id)
  (call-api "/api/v2/move/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec move-retrieve move-detail-spec
  "Get move
  Moves are the skills of Pokémon in battle. In battle, a Pokémon uses one move each turn. Some moves (including those learned by Hidden Machine) can be used outside of battle as well, usually for the purpose of removing obstacles or exploring new areas."
  [id string?]
  (let [res (:data (move-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode move-detail-spec res st/string-transformer)
       res)))


(defn-spec move-target-list-with-http-info any?
  "List move targets
  Targets moves can be directed at during battle. Targets can be Pokémon, environments or even other moves."
  ([] (move-target-list-with-http-info nil))
  ([{:keys [limit offset q]} (s/map-of keyword? any?)]
   (call-api "/api/v2/move-target/" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"limit" limit "offset" offset "q" q }
              :form-params   {}
...
(defn-spec move-target-list paginated-move-target-summary-list-spec
  "List move targets
  Targets moves can be directed at during battle. Targets can be Pokémon, environments or even other moves."
  ([] (move-target-list nil))
  ([optional-params any?]
   (let [res (:data (move-target-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-move-target-summary-list-spec res st/string-transformer)
        res))))


(defn-spec move-target-retrieve-with-http-info any?
  "Get move target
  Targets moves can be directed at during battle. Targets can be Pokémon, environments or even other moves."
  [id string?]
  (check-required-params id)
  (call-api "/api/v2/move-target/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec move-target-retrieve move-target-detail-spec
  "Get move target
  Targets moves can be directed at during battle. Targets can be Pokémon, environments or even other moves."
  [id string?]
  (let [res (:data (move-target-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode move-target-detail-spec res st/string-transformer)
       res)))
