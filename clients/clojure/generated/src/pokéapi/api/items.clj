...
(defn-spec item-attribute-list paginated-item-attribute-summary-list-spec
  "List item attributes
  Item attributes define particular aspects of items, e.g.\"usable in battle\" or \"consumable\"."
  ...
  ([optional-params any?]
   (let [res (:data (item-attribute-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-item-attribute-summary-list-spec res st/string-transformer)
        res))))


(defn-spec item-attribute-retrieve-with-http-info any?
  "Get item attribute
  Item attributes define particular aspects of items, e.g.\"usable in battle\" or \"consumable\"."
  [id string?]
...
(defn-spec item-attribute-retrieve item-attribute-detail-spec
  "Get item attribute
  Item attributes define particular aspects of items, e.g.\"usable in battle\" or \"consumable\"."
  [id string?]
  (let [res (:data (item-attribute-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode item-attribute-detail-spec res st/string-transformer)
       res)))


(defn-spec item-category-list-with-http-info any?
  "List item categories
  Item categories determine where items will be placed in the players bag."
  ([] (item-category-list-with-http-info nil))
  ([{:keys [limit offset q]} (s/map-of keyword? any?)]
   (call-api "/api/v2/item-category/" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"limit" limit "offset" offset "q" q }
              :form-params   {}
...
(defn-spec item-category-list paginated-item-category-summary-list-spec
  "List item categories
  Item categories determine where items will be placed in the players bag."
  ([] (item-category-list nil))
  ([optional-params any?]
   (let [res (:data (item-category-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-item-category-summary-list-spec res st/string-transformer)
        res))))


(defn-spec item-category-retrieve-with-http-info any?
  "Get item category
  Item categories determine where items will be placed in the players bag."
  [id string?]
  (check-required-params id)
  (call-api "/api/v2/item-category/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec item-category-retrieve item-category-detail-spec
  "Get item category
  Item categories determine where items will be placed in the players bag."
  [id string?]
  (let [res (:data (item-category-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode item-category-detail-spec res st/string-transformer)
       res)))
...
(defn-spec item-fling-effect-list paginated-item-fling-effect-summary-list-spec
  "List item fling effects
  The various effects of the move\"Fling\" when used with different items."
  ...
  ([optional-params any?]
   (let [res (:data (item-fling-effect-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-item-fling-effect-summary-list-spec res st/string-transformer)
        res))))


(defn-spec item-fling-effect-retrieve-with-http-info any?
...
(defn-spec item-fling-effect-retrieve item-fling-effect-detail-spec
  "Get item fling effect
  The various effects of the move\"Fling\" when used with different items."
  [id string?]
  (let [res (:data (item-fling-effect-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode item-fling-effect-detail-spec res st/string-transformer)
       res)))
...
(defn-spec item-list paginated-item-summary-list-spec
  "List items
  An item is an object in the games which the player can pick up, keep in their bag, and use in some manner. They have various uses, including healing, powering up, helping catch Pokémon, or to access a new area."
  ([] (item-list nil))
  ([optional-params any?]
   (let [res (:data (item-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-item-summary-list-spec res st/string-transformer)
        res))))
...
(defn-spec item-pocket-list paginated-item-pocket-summary-list-spec
  "List item pockets
  Pockets within the players bag used for storing items by category."
  ...
  ([optional-params any?]
   (let [res (:data (item-pocket-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-item-pocket-summary-list-spec res st/string-transformer)
        res))))


(defn-spec item-pocket-retrieve-with-http-info any?
  "Get item pocket
  Pockets within the players bag used for storing items by category."
  [id string?]
  (check-required-params id)
...
(defn-spec item-pocket-retrieve item-pocket-detail-spec
  "Get item pocket
  Pockets within the players bag used for storing items by category."
  [id string?]
  (let [res (:data (item-pocket-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode item-pocket-detail-spec res st/string-transformer)
       res)))


(defn-spec item-retrieve-with-http-info any?
  "Get item
  An item is an object in the games which the player can pick up, keep in their bag, and use in some manner. They have various uses, including healing, powering up, helping catch Pokémon, or to access a new area."
  [id string?]
  (check-required-params id)
...
(defn-spec item-retrieve item-detail-spec
  "Get item
  An item is an object in the games which the player can pick up, keep in their bag, and use in some manner. They have various uses, including healing, powering up, helping catch Pokémon, or to access a new area."
  [id string?]
  (let [res (:data (item-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode item-detail-spec res st/string-transformer)
       res)))
