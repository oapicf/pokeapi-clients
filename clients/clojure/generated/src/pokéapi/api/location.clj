...
(defn-spec location-area-list-with-http-info any?
  "List location areas
  Location areas are sections of areas, such as floors in a building or cave. Each area has its own set of possible Pokémon encounters."
  ([] (location-area-list-with-http-info nil))
  ([{:keys [limit offset]} (s/map-of keyword? any?)]
   (call-api "/api/v2/location-area/" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec location-area-list paginated-location-area-summary-list-spec
  "List location areas
  Location areas are sections of areas, such as floors in a building or cave. Each area has its own set of possible Pokémon encounters."
  ([] (location-area-list nil))
  ([optional-params any?]
   (let [res (:data (location-area-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-location-area-summary-list-spec res st/string-transformer)
        res))))


(defn-spec location-area-retrieve-with-http-info any?
  "Get location area
  Location areas are sections of areas, such as floors in a building or cave. Each area has its own set of possible Pokémon encounters."
  [id int?]
  (check-required-params id)
  (call-api "/api/v2/location-area/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec location-area-retrieve location-area-detail-spec
  "Get location area
  Location areas are sections of areas, such as floors in a building or cave. Each area has its own set of possible Pokémon encounters."
  [id int?]
  (let [res (:data (location-area-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode location-area-detail-spec res st/string-transformer)
       res)))


(defn-spec location-list-with-http-info any?
  "List locations
  Locations that can be visited within the games. Locations make up sizable portions of regions, like cities or routes."
  ([] (location-list-with-http-info nil))
  ([{:keys [limit offset q]} (s/map-of keyword? any?)]
   (call-api "/api/v2/location/" :get
             {:path-params   {}
              :header-params {}
...
(defn-spec location-list paginated-location-summary-list-spec
  "List locations
  Locations that can be visited within the games. Locations make up sizable portions of regions, like cities or routes."
  ([] (location-list nil))
  ([optional-params any?]
   (let [res (:data (location-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-location-summary-list-spec res st/string-transformer)
        res))))


(defn-spec location-retrieve-with-http-info any?
  "Get location
  Locations that can be visited within the games. Locations make up sizable portions of regions, like cities or routes."
  [id string?]
  (check-required-params id)
  (call-api "/api/v2/location/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec location-retrieve location-detail-spec
  "Get location
  Locations that can be visited within the games. Locations make up sizable portions of regions, like cities or routes."
  [id string?]
  (let [res (:data (location-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode location-detail-spec res st/string-transformer)
       res)))
...
(defn-spec pal-park-area-list paginated-pal-park-area-summary-list-spec
  "List pal park areas
  Areas used for grouping Pokémon encounters in Pal Park. They're like habitats that are specific to Pal Park."
  ...
  ([optional-params any?]
   (let [res (:data (pal-park-area-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-pal-park-area-summary-list-spec res st/string-transformer)
        res))))


(defn-spec pal-park-area-retrieve-with-http-info any?
  "Get pal park area
  Areas used for grouping Pokémon encounters in Pal Park. They're like habitats that are specific to Pal Park."
  [id string?]
  (check-required-params id)
...
(defn-spec pal-park-area-retrieve pal-park-area-detail-spec
  "Get pal park area
  Areas used for grouping Pokémon encounters in Pal Park. They're like habitats that are specific to Pal Park."
  [id string?]
  (let [res (:data (pal-park-area-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode pal-park-area-detail-spec res st/string-transformer)
       res)))


(defn-spec region-list-with-http-info any?
  "List regions
  A region is an organized area of the Pokémon world. Most often, the main difference between regions is the species of Pokémon that can be encountered within them."
  ([] (region-list-with-http-info nil))
  ([{:keys [limit offset q]} (s/map-of keyword? any?)]
   (call-api "/api/v2/region/" :get
             {:path-params   {}
              :header-params {}
              :query-params  {"limit" limit "offset" offset "q" q }
              :form-params   {}
...
(defn-spec region-list paginated-region-summary-list-spec
  "List regions
  A region is an organized area of the Pokémon world. Most often, the main difference between regions is the species of Pokémon that can be encountered within them."
  ([] (region-list nil))
  ([optional-params any?]
   (let [res (:data (region-list-with-http-info optional-params))]
     (if (:decode-models *api-context*)
        (st/decode paginated-region-summary-list-spec res st/string-transformer)
        res))))


(defn-spec region-retrieve-with-http-info any?
  "Get region
  A region is an organized area of the Pokémon world. Most often, the main difference between regions is the species of Pokémon that can be encountered within them."
  [id string?]
  (check-required-params id)
  (call-api "/api/v2/region/{id}/" :get
            {:path-params   {"id" id }
             :header-params {}
             :query-params  {}
             :form-params   {}
...
(defn-spec region-retrieve region-detail-spec
  "Get region
  A region is an organized area of the Pokémon world. Most often, the main difference between regions is the species of Pokémon that can be encountered within them."
  [id string?]
  (let [res (:data (region-retrieve-with-http-info id))]
    (if (:decode-models *api-context*)
       (st/decode region-detail-spec res st/string-transformer)
       res)))
