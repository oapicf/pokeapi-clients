#' Create a new AbilityEffectText
#'
#' @description
#' AbilityEffectText Class
#'
#' @docType class
#' @title AbilityEffectText
#' @description AbilityEffectText Class
...
AbilityEffectText <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new AbilityEffectText class.
    #'
    #' @param effect effect
    ...
    #'
    #' @return AbilityEffectText as a base R list.
    #' @examples
    #' # convert array of AbilityEffectText (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert AbilityEffectText to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      AbilityEffectTextObject <- list()
      if (!is.null(self$`effect`)) {
        AbilityEffectTextObject[["effect"]] <-
          self$`effect`
      }
      if (!is.null(self$`short_effect`)) {
        AbilityEffectTextObject[["short_effect"]] <-
          self$`short_effect`
      }
      if (!is.null(self$`language`)) {
        AbilityEffectTextObject[["language"]] <-
          self$`language`$toSimpleType()
      }
      return(AbilityEffectTextObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of AbilityEffectText
    #'
    #' @param input_json the JSON input
    #' @return the instance of AbilityEffectText
    ...
    #' @return AbilityEffectText in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to AbilityEffectText and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`effect`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for AbilityEffectText: the required field `effect` is missing."))
      }
      ...
      if (!is.null(input_json$`short_effect`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for AbilityEffectText: the required field `short_effect` is missing."))
      }
      # check the required field `language`
      if (!is.null(input_json$`language`)) {
        stopifnot(R6::is.R6(input_json$`language`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for AbilityEffectText: the required field `language` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of AbilityEffectText
    toString = function() {
      self$toJSONString()
    },
...
# AbilityEffectText$unlock()
#
## Below is an example to define the print function
# AbilityEffectText$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# AbilityEffectText$lock()
