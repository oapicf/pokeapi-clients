...
#'
#' @description
#' MoveFlavorText Class
#'
#' @docType class
#' @title MoveFlavorText
#' @description MoveFlavorText Class
...
MoveFlavorText <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new MoveFlavorText class.
    #'
    ...
    #'
    #' @return MoveFlavorText as a base R list.
    #' @examples
    #' # convert array of MoveFlavorText (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert MoveFlavorText to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      MoveFlavorTextObject <- list()
      if (!is.null(self$`flavor_text`)) {
        MoveFlavorTextObject[["flavor_text"]] <-
          self$`flavor_text`
      }
      if (!is.null(self$`language`)) {
        MoveFlavorTextObject[["language"]] <-
          self$`language`$toSimpleType()
      }
      if (!is.null(self$`version_group`)) {
        MoveFlavorTextObject[["version_group"]] <-
          self$`version_group`$toSimpleType()
      }
      return(MoveFlavorTextObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of MoveFlavorText
    #'
    #' @param input_json the JSON input
    #' @return the instance of MoveFlavorText
    ...
    #' @return MoveFlavorText in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to MoveFlavorText and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`flavor_text`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for MoveFlavorText: the required field `flavor_text` is missing."))
      }
      # check the required field `language`
      if (!is.null(input_json$`language`)) {
        stopifnot(R6::is.R6(input_json$`language`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for MoveFlavorText: the required field `language` is missing."))
      }
      # check the required field `version_group`
      if (!is.null(input_json$`version_group`)) {
        stopifnot(R6::is.R6(input_json$`version_group`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for MoveFlavorText: the required field `version_group` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of MoveFlavorText
    toString = function() {
      self$toJSONString()
    },
...
# MoveFlavorText$unlock()
#
## Below is an example to define the print function
# MoveFlavorText$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
