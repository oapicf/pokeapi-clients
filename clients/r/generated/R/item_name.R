...
ItemName <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new ItemName class.
    #'
    #' @param name name
    ...
    #'
    #' @return ItemName as a base R list.
    #' @examples
    #' # convert array of ItemName (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert ItemName to a base R type
    #'
    ...
    #' @description
    #' Deserialize JSON string into an instance of ItemName
    #'
    #' @param input_json the JSON input
    #' @return the instance of ItemName
    ...
    #' @return ItemName in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to ItemName and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for ItemName: the required field `name` is missing."))
      }
      # check the required field `language`
      if (!is.null(input_json$`language`)) {
        stopifnot(R6::is.R6(input_json$`language`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for ItemName: the required field `language` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of ItemName
    toString = function() {
      self$toJSONString()
    },
...
# ItemName$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
