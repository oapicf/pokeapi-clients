#' Create a new PokemonSpeciesFlavorText
#'
#' @description
#' PokemonSpeciesFlavorText Class
#'
#' @docType class
#' @title PokemonSpeciesFlavorText
#' @description PokemonSpeciesFlavorText Class
...
#' @export
PokemonSpeciesFlavorText <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new PokemonSpeciesFlavorText class.
    #'
    #' @param flavor_text flavor_text
    #' @param language language
    #' @param version version
    #' @param ... Other optional arguments.
    initialize = function(`flavor_text`, `language`, `version`, ...) {
    ...
    #'
    #' @return PokemonSpeciesFlavorText as a base R list.
    #' @examples
    #' # convert array of PokemonSpeciesFlavorText (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert PokemonSpeciesFlavorText to a base R type
    #'
    #' @return A base R type, e.g. a list or numeric/character array.
    toSimpleType = function() {
      PokemonSpeciesFlavorTextObject <- list()
      if (!is.null(self$`flavor_text`)) {
        PokemonSpeciesFlavorTextObject[["flavor_text"]] <-
          self$`flavor_text`
      }
      if (!is.null(self$`language`)) {
        PokemonSpeciesFlavorTextObject[["language"]] <-
          self$`language`$toSimpleType()
      }
      if (!is.null(self$`version`)) {
        PokemonSpeciesFlavorTextObject[["version"]] <-
          self$`version`$toSimpleType()
      }
      return(PokemonSpeciesFlavorTextObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of PokemonSpeciesFlavorText
    #'
    #' @param input_json the JSON input
    #' @return the instance of PokemonSpeciesFlavorText
    ...
    #' @return PokemonSpeciesFlavorText in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to PokemonSpeciesFlavorText and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`flavor_text`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for PokemonSpeciesFlavorText: the required field `flavor_text` is missing."))
      }
      # check the required field `language`
      if (!is.null(input_json$`language`)) {
        stopifnot(R6::is.R6(input_json$`language`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for PokemonSpeciesFlavorText: the required field `language` is missing."))
      }
      # check the required field `version`
      if (!is.null(input_json$`version`)) {
        stopifnot(R6::is.R6(input_json$`version`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for PokemonSpeciesFlavorText: the required field `version` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of PokemonSpeciesFlavorText
    toString = function() {
      self$toJSONString()
    },
...
# PokemonSpeciesFlavorText$unlock()
#
## Below is an example to define the print function
# PokemonSpeciesFlavorText$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
## Uncomment below to lock the class to prevent modifications to the method or field
# PokemonSpeciesFlavorText$lock()
