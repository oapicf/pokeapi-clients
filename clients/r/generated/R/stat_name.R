...
StatName <- R6::R6Class(
  ...
  public = list(
    ...
    #' @description
    #' Initialize a new StatName class.
    #'
    #' @param name name
    ...
    #'
    #' @return StatName as a base R list.
    #' @examples
    #' # convert array of StatName (x) to a data frame
    #' \dontrun{
    #' library(purrr)
    #' library(tibble)
    ...
    #' @description
    #' Convert StatName to a base R type
    #'
    ...
    toSimpleType = function() {
      ...
      if (!is.null(self$`language`)) {
        StatNameObject[["language"]] <-
          self$`language`$toSimpleType()
      }
      return(StatNameObject)
    },

    #' @description
    #' Deserialize JSON string into an instance of StatName
    #'
    #' @param input_json the JSON input
    #' @return the instance of StatName
    ...
    #' @return StatName in JSON format
    ...
    #' @description
    #' Validate JSON input with respect to StatName and throw an exception if invalid
    #'
    ...
    validateJSON = function(input) {
      ...
      if (!is.null(input_json$`name`)) {
        ...
        stop(paste("The JSON input `", input, "` is invalid for StatName: the required field `name` is missing."))
      }
      # check the required field `language`
      if (!is.null(input_json$`language`)) {
        stopifnot(R6::is.R6(input_json$`language`))
      } else {
        stop(paste("The JSON input `", input, "` is invalid for StatName: the required field `language` is missing."))
      }
    },
    ...
    #'
    #' @return String representation of StatName
    toString = function() {
      self$toJSONString()
    },
...
# StatName$set("public", "print", function(...) {
#   print(jsonlite::prettify(self$toJSONString()))
#   invisible(self)
# })
