...
ContestTypeName<PSCustomObject>
#>

function Initialize-ContestTypeName {
    [CmdletBinding()]
    Param (
    ...
    Process {
        'Creating PSCustomObject: PSOpenAPITools => ContestTypeName' | Write-Debug
        ...
        if ($Color.length -gt 10) {
            throw "invalid value for 'Color', the character length must be smaller than or equal to 10."
        }
...
Convert from JSON to ContestTypeName<PSCustomObject>
...
ContestTypeName<PSCustomObject>
#>
function ConvertFrom-JsonToContestTypeName {
    Param(
        [AllowEmptyString()]
        [string]$Json
    )
    ...
    Process {
        'Converting JSON to PSCustomObject: PSOpenAPITools => ContestTypeName' | Write-Debug
        $PSBoundParameters | Out-DebugParameter | Write-Debug
