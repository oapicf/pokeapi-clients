...
------------- | ------------- | -------------
[**Invoke-LocationList**](LocationApi.md#Invoke-LocationList) | **GET** /api/v2/location/ | 
...
try {
    $Result = Invoke-LocationList -Limit $Limit -Offset $Offset
} catch {
...
try {
    $Result = Invoke-LocationRead -Id $Id
} catch {
    Write-Host ("Exception occurred when calling Invoke-LocationRead: {0}" -f ($_.ErrorDetails | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
```
