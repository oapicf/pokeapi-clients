...
------------- | ------------- | -------------
[**Invoke-ItemList**](ItemApi.md#Invoke-ItemList) | **GET** /api/v2/item/ | 
...
try {
    $Result = Invoke-ItemList -Limit $Limit -Offset $Offset
} catch {
...
try {
    $Result = Invoke-ItemRead -Id $Id
} catch {
    Write-Host ("Exception occurred when calling Invoke-ItemRead: {0}" -f ($_.ErrorDetails | ConvertFrom-Json))
    Write-Host ("Response headers: {0}" -f ($_.Exception.Response.Headers | ConvertTo-Json))
}
```
