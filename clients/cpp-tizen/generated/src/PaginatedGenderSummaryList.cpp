...
void
PaginatedGenderSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedGenderSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedGenderSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<GenderSummary> new_list;
			GenderSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("GenderSummary")) {
					jsonToValue(&inst, temp_json, "GenderSummary", "");
				} else {
...
PaginatedGenderSummaryList::PaginatedGenderSummaryList(char* json)
{
	this->fromJson(json);
}

char*
PaginatedGenderSummaryList::toJson()
{
	...
	if (isprimitive("GenderSummary")) {
		list<GenderSummary> new_list = static_cast<list <GenderSummary> > (getResults());
		node = converttoJson(&new_list, "GenderSummary", "array");
	} else {
		...
		for (list<GenderSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			GenderSummary obj = *it;
...
int
PaginatedGenderSummaryList::getCount()
{
	return count;
}
...
std::string
PaginatedGenderSummaryList::getNext()
{
	return next;
}
...
std::string
PaginatedGenderSummaryList::getPrevious()
{
	return previous;
}
...
std::list<GenderSummary>
PaginatedGenderSummaryList::getResults()
{
	return results;
}
