...
void
EvolutionChainDetail_chain_evolves_to_inner::__init()
{
...
void
EvolutionChainDetail_chain_evolves_to_inner::__cleanup()
{
...
void
EvolutionChainDetail_chain_evolves_to_inner::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner> new_list;
			EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner")) {
					jsonToValue(&inst, temp_json, "EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner", "");
				} else {
...
EvolutionChainDetail_chain_evolves_to_inner::EvolutionChainDetail_chain_evolves_to_inner(char* json)
{
	this->fromJson(json);
}
...
EvolutionChainDetail_chain_evolves_to_inner::toJson()
{
	...
	JsonNode *node;
	if (isprimitive("EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner")) {
		list<EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner> new_list = static_cast<list <EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner> > (getEvolutionDetails());
		node = converttoJson(&new_list, "EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner", "array");
	} else {
		...
		for (list<EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			EvolutionChainDetail_chain_evolves_to_inner_evolution_details_inner obj = *it;
...
bool
EvolutionChainDetail_chain_evolves_to_inner::getIsBaby()
{
	return is_baby;
}
