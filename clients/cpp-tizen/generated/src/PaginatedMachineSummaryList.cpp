...
void
PaginatedMachineSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedMachineSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedMachineSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<MachineSummary> new_list;
			MachineSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("MachineSummary")) {
					jsonToValue(&inst, temp_json, "MachineSummary", "");
				} else {
...
PaginatedMachineSummaryList::PaginatedMachineSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedMachineSummaryList::toJson()
{
	...
	if (isprimitive("MachineSummary")) {
		list<MachineSummary> new_list = static_cast<list <MachineSummary> > (getResults());
		node = converttoJson(&new_list, "MachineSummary", "array");
	} else {
		...
		for (list<MachineSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			MachineSummary obj = *it;
