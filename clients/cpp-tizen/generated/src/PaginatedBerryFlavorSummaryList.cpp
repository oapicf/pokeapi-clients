...
void
PaginatedBerryFlavorSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedBerryFlavorSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedBerryFlavorSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<BerryFlavorSummary> new_list;
			BerryFlavorSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("BerryFlavorSummary")) {
					jsonToValue(&inst, temp_json, "BerryFlavorSummary", "");
				} else {
...
PaginatedBerryFlavorSummaryList::PaginatedBerryFlavorSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedBerryFlavorSummaryList::toJson()
{
	...
	if (isprimitive("BerryFlavorSummary")) {
		list<BerryFlavorSummary> new_list = static_cast<list <BerryFlavorSummary> > (getResults());
		node = converttoJson(&new_list, "BerryFlavorSummary", "array");
	} else {
		...
		for (list<BerryFlavorSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			BerryFlavorSummary obj = *it;
