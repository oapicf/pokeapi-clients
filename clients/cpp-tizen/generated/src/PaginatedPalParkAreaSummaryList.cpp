...
void
PaginatedPalParkAreaSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedPalParkAreaSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedPalParkAreaSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<PalParkAreaSummary> new_list;
			PalParkAreaSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("PalParkAreaSummary")) {
					jsonToValue(&inst, temp_json, "PalParkAreaSummary", "");
				} else {
...
PaginatedPalParkAreaSummaryList::PaginatedPalParkAreaSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedPalParkAreaSummaryList::toJson()
{
	...
	if (isprimitive("PalParkAreaSummary")) {
		list<PalParkAreaSummary> new_list = static_cast<list <PalParkAreaSummary> > (getResults());
		node = converttoJson(&new_list, "PalParkAreaSummary", "array");
	} else {
		...
		for (list<PalParkAreaSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			PalParkAreaSummary obj = *it;
