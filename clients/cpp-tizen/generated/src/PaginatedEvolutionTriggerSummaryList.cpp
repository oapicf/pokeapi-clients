...
void
PaginatedEvolutionTriggerSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedEvolutionTriggerSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedEvolutionTriggerSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<EvolutionTriggerSummary> new_list;
			EvolutionTriggerSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("EvolutionTriggerSummary")) {
					jsonToValue(&inst, temp_json, "EvolutionTriggerSummary", "");
				} else {
...
PaginatedEvolutionTriggerSummaryList::PaginatedEvolutionTriggerSummaryList(char* json)
{
	this->fromJson(json);
}

char*
PaginatedEvolutionTriggerSummaryList::toJson()
{
	...
	if (isprimitive("EvolutionTriggerSummary")) {
		list<EvolutionTriggerSummary> new_list = static_cast<list <EvolutionTriggerSummary> > (getResults());
		node = converttoJson(&new_list, "EvolutionTriggerSummary", "array");
	} else {
		...
		for (list<EvolutionTriggerSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			EvolutionTriggerSummary obj = *it;
...
int
PaginatedEvolutionTriggerSummaryList::getCount()
{
	return count;
}
...
std::string
PaginatedEvolutionTriggerSummaryList::getNext()
{
	return next;
}
...
std::string
PaginatedEvolutionTriggerSummaryList::getPrevious()
{
	return previous;
}
...
std::list<EvolutionTriggerSummary>
PaginatedEvolutionTriggerSummaryList::getResults()
{
	return results;
}
