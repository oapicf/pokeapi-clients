...
void
PaginatedSuperContestEffectSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedSuperContestEffectSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedSuperContestEffectSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<SuperContestEffectSummary> new_list;
			SuperContestEffectSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("SuperContestEffectSummary")) {
					jsonToValue(&inst, temp_json, "SuperContestEffectSummary", "");
				} else {
...
PaginatedSuperContestEffectSummaryList::PaginatedSuperContestEffectSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedSuperContestEffectSummaryList::toJson()
{
	...
	if (isprimitive("SuperContestEffectSummary")) {
		list<SuperContestEffectSummary> new_list = static_cast<list <SuperContestEffectSummary> > (getResults());
		node = converttoJson(&new_list, "SuperContestEffectSummary", "array");
	} else {
		...
		for (list<SuperContestEffectSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			SuperContestEffectSummary obj = *it;
