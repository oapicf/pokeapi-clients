...
void
PaginatedContestEffectSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedContestEffectSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedContestEffectSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ContestEffectSummary> new_list;
			ContestEffectSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ContestEffectSummary")) {
					jsonToValue(&inst, temp_json, "ContestEffectSummary", "");
				} else {
...
PaginatedContestEffectSummaryList::PaginatedContestEffectSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedContestEffectSummaryList::toJson()
{
	...
	if (isprimitive("ContestEffectSummary")) {
		list<ContestEffectSummary> new_list = static_cast<list <ContestEffectSummary> > (getResults());
		node = converttoJson(&new_list, "ContestEffectSummary", "array");
	} else {
		...
		for (list<ContestEffectSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ContestEffectSummary obj = *it;
