...
void
PaginatedEggGroupSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedEggGroupSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedEggGroupSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<EggGroupSummary> new_list;
			EggGroupSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("EggGroupSummary")) {
					jsonToValue(&inst, temp_json, "EggGroupSummary", "");
				} else {
...
PaginatedEggGroupSummaryList::PaginatedEggGroupSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedEggGroupSummaryList::toJson()
{
	...
	if (isprimitive("EggGroupSummary")) {
		list<EggGroupSummary> new_list = static_cast<list <EggGroupSummary> > (getResults());
		node = converttoJson(&new_list, "EggGroupSummary", "array");
	} else {
		...
		for (list<EggGroupSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			EggGroupSummary obj = *it;
