...
void
MoveBattleStyleDetail::__cleanup()
{
	//if(id != NULL) {
	//
	//delete id;
...
void
MoveBattleStyleDetail::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<MoveBattleStyleName> new_list;
			MoveBattleStyleName inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("MoveBattleStyleName")) {
					jsonToValue(&inst, temp_json, "MoveBattleStyleName", "");
				} else {
...
MoveBattleStyleDetail::MoveBattleStyleDetail(char* json)
{
	this->fromJson(json);
}
...
MoveBattleStyleDetail::toJson()
{
	...
	if (isprimitive("MoveBattleStyleName")) {
		list<MoveBattleStyleName> new_list = static_cast<list <MoveBattleStyleName> > (getNames());
		node = converttoJson(&new_list, "MoveBattleStyleName", "array");
	} else {
		...
		for (list<MoveBattleStyleName>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			MoveBattleStyleName obj = *it;
