...
void
PaginatedLanguageSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedLanguageSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedLanguageSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<LanguageSummary> new_list;
			LanguageSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("LanguageSummary")) {
					jsonToValue(&inst, temp_json, "LanguageSummary", "");
				} else {
...
PaginatedLanguageSummaryList::PaginatedLanguageSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedLanguageSummaryList::toJson()
{
	...
	if (isprimitive("LanguageSummary")) {
		list<LanguageSummary> new_list = static_cast<list <LanguageSummary> > (getResults());
		node = converttoJson(&new_list, "LanguageSummary", "array");
	} else {
		...
		for (list<LanguageSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			LanguageSummary obj = *it;
