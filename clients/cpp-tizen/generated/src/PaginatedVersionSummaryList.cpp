...
void
PaginatedVersionSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedVersionSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedVersionSummaryList::fromJson(char* jsonStr)
{
...
PaginatedVersionSummaryList::PaginatedVersionSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedVersionSummaryList::toJson()
{
	...
	if (isprimitive("VersionSummary")) {
		list<VersionSummary> new_list = static_cast<list <VersionSummary> > (getResults());
