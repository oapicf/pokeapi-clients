...
void
PaginatedContestTypeSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedContestTypeSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedContestTypeSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<ContestTypeSummary> new_list;
			ContestTypeSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("ContestTypeSummary")) {
					jsonToValue(&inst, temp_json, "ContestTypeSummary", "");
				} else {
...
PaginatedContestTypeSummaryList::PaginatedContestTypeSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedContestTypeSummaryList::toJson()
{
	...
	if (isprimitive("ContestTypeSummary")) {
		list<ContestTypeSummary> new_list = static_cast<list <ContestTypeSummary> > (getResults());
		node = converttoJson(&new_list, "ContestTypeSummary", "array");
	} else {
		...
		for (list<ContestTypeSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			ContestTypeSummary obj = *it;
