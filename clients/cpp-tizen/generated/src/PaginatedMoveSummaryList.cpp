...
void
PaginatedMoveSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedMoveSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedMoveSummaryList::fromJson(char* jsonStr)
{
...
PaginatedMoveSummaryList::PaginatedMoveSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedMoveSummaryList::toJson()
{
	...
	if (isprimitive("MoveSummary")) {
		list<MoveSummary> new_list = static_cast<list <MoveSummary> > (getResults());
