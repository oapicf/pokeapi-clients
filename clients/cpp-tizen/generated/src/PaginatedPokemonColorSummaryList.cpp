...
void
PaginatedPokemonColorSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedPokemonColorSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedPokemonColorSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<PokemonColorSummary> new_list;
			PokemonColorSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("PokemonColorSummary")) {
					jsonToValue(&inst, temp_json, "PokemonColorSummary", "");
				} else {
...
PaginatedPokemonColorSummaryList::PaginatedPokemonColorSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedPokemonColorSummaryList::toJson()
{
	...
	if (isprimitive("PokemonColorSummary")) {
		list<PokemonColorSummary> new_list = static_cast<list <PokemonColorSummary> > (getResults());
		node = converttoJson(&new_list, "PokemonColorSummary", "array");
	} else {
		...
		for (list<PokemonColorSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			PokemonColorSummary obj = *it;
