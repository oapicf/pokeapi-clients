...
void
PaginatedPokemonSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedPokemonSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedPokemonSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<PokemonSummary> new_list;
			PokemonSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("PokemonSummary")) {
					jsonToValue(&inst, temp_json, "PokemonSummary", "");
				} else {
...
PaginatedPokemonSummaryList::PaginatedPokemonSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedPokemonSummaryList::toJson()
{
	...
	if (isprimitive("PokemonSummary")) {
		list<PokemonSummary> new_list = static_cast<list <PokemonSummary> > (getResults());
		node = converttoJson(&new_list, "PokemonSummary", "array");
	} else {
		...
		for (list<PokemonSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			PokemonSummary obj = *it;
