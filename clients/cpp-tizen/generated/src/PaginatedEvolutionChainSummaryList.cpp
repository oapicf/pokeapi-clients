...
void
PaginatedEvolutionChainSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedEvolutionChainSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedEvolutionChainSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<EvolutionChainSummary> new_list;
			EvolutionChainSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("EvolutionChainSummary")) {
					jsonToValue(&inst, temp_json, "EvolutionChainSummary", "");
				} else {
...
PaginatedEvolutionChainSummaryList::PaginatedEvolutionChainSummaryList(char* json)
{
	this->fromJson(json);
}

char*
PaginatedEvolutionChainSummaryList::toJson()
{
	...
	if (isprimitive("EvolutionChainSummary")) {
		list<EvolutionChainSummary> new_list = static_cast<list <EvolutionChainSummary> > (getResults());
		node = converttoJson(&new_list, "EvolutionChainSummary", "array");
	} else {
		...
		for (list<EvolutionChainSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			EvolutionChainSummary obj = *it;
...
int
PaginatedEvolutionChainSummaryList::getCount()
{
	return count;
}
...
std::string
PaginatedEvolutionChainSummaryList::getNext()
{
	return next;
}
...
std::string
PaginatedEvolutionChainSummaryList::getPrevious()
{
	return previous;
}
...
std::list<EvolutionChainSummary>
PaginatedEvolutionChainSummaryList::getResults()
{
	return results;
}
