...
void
PaginatedBerryFirmnessSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedBerryFirmnessSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedBerryFirmnessSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<BerryFirmnessSummary> new_list;
			BerryFirmnessSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("BerryFirmnessSummary")) {
					jsonToValue(&inst, temp_json, "BerryFirmnessSummary", "");
				} else {
...
PaginatedBerryFirmnessSummaryList::PaginatedBerryFirmnessSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedBerryFirmnessSummaryList::toJson()
{
	...
	if (isprimitive("BerryFirmnessSummary")) {
		list<BerryFirmnessSummary> new_list = static_cast<list <BerryFirmnessSummary> > (getResults());
		node = converttoJson(&new_list, "BerryFirmnessSummary", "array");
	} else {
		...
		for (list<BerryFirmnessSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			BerryFirmnessSummary obj = *it;
