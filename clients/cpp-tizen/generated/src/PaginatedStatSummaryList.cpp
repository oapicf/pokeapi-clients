...
void
PaginatedStatSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedStatSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedStatSummaryList::fromJson(char* jsonStr)
{
	...
	if (node !=NULL) {
	
		{
			...
			JsonNode*  temp_json;
			list<StatSummary> new_list;
			StatSummary inst;
			for (guint i=0;i<json_array_get_length(arr);i++) {
				temp_json = json_array_get_element(arr,i);
				if (isprimitive("StatSummary")) {
					jsonToValue(&inst, temp_json, "StatSummary", "");
				} else {
...
PaginatedStatSummaryList::PaginatedStatSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedStatSummaryList::toJson()
{
	...
	if (isprimitive("StatSummary")) {
		list<StatSummary> new_list = static_cast<list <StatSummary> > (getResults());
		node = converttoJson(&new_list, "StatSummary", "array");
	} else {
		...
		for (list<StatSummary>::iterator it = new_list.begin(); it != new_list.end(); it++) {
			mygerror = NULL;
			StatSummary obj = *it;
