...
void
PaginatedItemSummaryList::__init()
{
	//count = int(0);
	//next = std::string();
	//previous = std::string();
...
void
PaginatedItemSummaryList::__cleanup()
{
	//if(count != NULL) {
	//
	//delete count;
...
void
PaginatedItemSummaryList::fromJson(char* jsonStr)
{
...
PaginatedItemSummaryList::PaginatedItemSummaryList(char* json)
{
	this->fromJson(json);
}
...
PaginatedItemSummaryList::toJson()
{
	...
	if (isprimitive("ItemSummary")) {
		list<ItemSummary> new_list = static_cast<list <ItemSummary> > (getResults());
