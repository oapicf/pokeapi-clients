...
const std::string LocationApi::base = "/api/v2";
...
void LocationApi::setupRoutes() {
    ...
    Routes::Get(*router, base + "/api/v2/location/:id/", Routes::bind(&LocationApi::location_read_handler, this));
...
void LocationApi::location_list_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    ...
    try {
        this->location_list(limit, offset, response);
...
void LocationApi::location_read_handler(const Pistache::Rest::Request &request, Pistache::Http::ResponseWriter response) {
    try {
    ...
    try {
        this->location_read(id, response);
    } catch (Pistache::Http::HttpError &e) {
        response.send(static_cast<Pistache::Http::Code>(e.code()), e.what());
        return;
    } catch (std::exception &e) {
