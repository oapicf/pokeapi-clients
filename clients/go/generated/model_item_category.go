/*


No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 20220523
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package openapi

import (
	"encoding/json"
)

// ItemCategory struct for ItemCategory
type ItemCategory struct {
	// The identifier for this item category resource
	Id *int32 `json:"id,omitempty"`
	// The name for this item category resource
	Name *string `json:"name,omitempty"`
	// A list of items that are a part of this category
	Items []NamedAPIResource `json:"items,omitempty"`
}

// NewItemCategory instantiates a new ItemCategory object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewItemCategory() *ItemCategory {
	this := ItemCategory{}
	return &this
}

// NewItemCategoryWithDefaults instantiates a new ItemCategory object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewItemCategoryWithDefaults() *ItemCategory {
	this := ItemCategory{}
	return &this
}

// GetId returns the Id field value if set, zero value otherwise.
func (o *ItemCategory) GetId() int32 {
	if o == nil || isNil(o.Id) {
		var ret int32
		return ret
	}
	return *o.Id
}

// GetIdOk returns a tuple with the Id field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ItemCategory) GetIdOk() (*int32, bool) {
	if o == nil || isNil(o.Id) {
    return nil, false
	}
	return o.Id, true
}

// HasId returns a boolean if a field has been set.
func (o *ItemCategory) HasId() bool {
	if o != nil && !isNil(o.Id) {
		return true
	}

	return false
}

// SetId gets a reference to the given int32 and assigns it to the Id field.
func (o *ItemCategory) SetId(v int32) {
	o.Id = &v
}

// GetName returns the Name field value if set, zero value otherwise.
func (o *ItemCategory) GetName() string {
	if o == nil || isNil(o.Name) {
		var ret string
		return ret
	}
	return *o.Name
}

// GetNameOk returns a tuple with the Name field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ItemCategory) GetNameOk() (*string, bool) {
	if o == nil || isNil(o.Name) {
    return nil, false
	}
	return o.Name, true
}

// HasName returns a boolean if a field has been set.
func (o *ItemCategory) HasName() bool {
	if o != nil && !isNil(o.Name) {
		return true
	}

	return false
}

// SetName gets a reference to the given string and assigns it to the Name field.
func (o *ItemCategory) SetName(v string) {
	o.Name = &v
}

// GetItems returns the Items field value if set, zero value otherwise.
func (o *ItemCategory) GetItems() []NamedAPIResource {
	if o == nil || isNil(o.Items) {
		var ret []NamedAPIResource
		return ret
	}
	return o.Items
}

// GetItemsOk returns a tuple with the Items field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *ItemCategory) GetItemsOk() ([]NamedAPIResource, bool) {
	if o == nil || isNil(o.Items) {
    return nil, false
	}
	return o.Items, true
}

// HasItems returns a boolean if a field has been set.
func (o *ItemCategory) HasItems() bool {
	if o != nil && !isNil(o.Items) {
		return true
	}

	return false
}

// SetItems gets a reference to the given []NamedAPIResource and assigns it to the Items field.
func (o *ItemCategory) SetItems(v []NamedAPIResource) {
	o.Items = v
}

func (o ItemCategory) MarshalJSON() ([]byte, error) {
	toSerialize := map[string]interface{}{}
	if !isNil(o.Id) {
		toSerialize["id"] = o.Id
	}
	if !isNil(o.Name) {
		toSerialize["name"] = o.Name
	}
	if !isNil(o.Items) {
		toSerialize["items"] = o.Items
	}
	return json.Marshal(toSerialize)
}

type NullableItemCategory struct {
	value *ItemCategory
	isSet bool
}

func (v NullableItemCategory) Get() *ItemCategory {
	return v.value
}

func (v *NullableItemCategory) Set(val *ItemCategory) {
	v.value = val
	v.isSet = true
}

func (v NullableItemCategory) IsSet() bool {
	return v.isSet
}

func (v *NullableItemCategory) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableItemCategory(val *ItemCategory) *NullableItemCategory {
	return &NullableItemCategory{value: val, isSet: true}
}

func (v NullableItemCategory) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableItemCategory) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


