...
pub async fn move_battle_style_read(configuration: &configuration::Configuration, id: i32) -> Result<String, Error<MoveBattleStyleReadError>> {
    ...
    let uri_str = format!("{}/api/v2/move-battle-style/{id}/", configuration.base_path, id=p_path_id);
    let mut req_builder = configuration.client.request(reqwest::Method::GET, &uri_str);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<MoveBattleStyleReadError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent { status, content, entity }))
    }
}
