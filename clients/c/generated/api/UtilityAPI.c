...
UtilityAPI_languageList(apiClient_t *apiClient, int *limit, int *offset, char *q)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v2/language/");
    ...
    paginated_language_summary_list_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *UtilityAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = paginated_language_summary_list_parseFromJSON(UtilityAPIlocalVarJSON);
        cJSON_Delete(UtilityAPIlocalVarJSON);
...
UtilityAPI_languageRetrieve(apiClient_t *apiClient, char *id)
{
    ...
    // create the path
    char *localVarPath = strdup("/api/v2/language/{id}/");
    ...
    //}
    //nonprimitive not container
    language_detail_t *elementToReturn = NULL;
    if(apiClient->response_code >= 200 && apiClient->response_code < 300) {
        cJSON *UtilityAPIlocalVarJSON = cJSON_Parse(apiClient->dataReceived);
        elementToReturn = language_detail_parseFromJSON(UtilityAPIlocalVarJSON);
        cJSON_Delete(UtilityAPIlocalVarJSON);
        if(elementToReturn == NULL) {
            // return 0;
        }
