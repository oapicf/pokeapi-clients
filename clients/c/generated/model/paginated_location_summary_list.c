...
#include <string.h>
#include <stdio.h>
#include "paginated_location_summary_list.h"
...
static paginated_location_summary_list_t *paginated_location_summary_list_create_internal(
    ...
    paginated_location_summary_list_t *paginated_location_summary_list_local_var = malloc(sizeof(paginated_location_summary_list_t));
...
void paginated_location_summary_list_free(paginated_location_summary_list_t *paginated_location_summary_list) {
    if(NULL == paginated_location_summary_list){
        return ;
    }
    if(paginated_location_summary_list->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "paginated_location_summary_list_free");
        return ;
    }
    ...
    if (paginated_location_summary_list->previous) {
        free(paginated_location_summary_list->previous);
        paginated_location_summary_list->previous = NULL;
    }
    if (paginated_location_summary_list->results) {
        list_ForEach(listEntry, paginated_location_summary_list->results) {
            location_summary_free(listEntry->data);
        }
        list_freeList(paginated_location_summary_list->results);
        paginated_location_summary_list->results = NULL;
    }
    free(paginated_location_summary_list);
}

cJSON *paginated_location_summary_list_convertToJSON(paginated_location_summary_list_t *paginated_location_summary_list) {
    ...
    if(paginated_location_summary_list->next) {
    if(cJSON_AddStringToObject(item, "next", paginated_location_summary_list->next) == NULL) {
    goto fail; //String
    }
    }


    // paginated_location_summary_list->previous
    if(paginated_location_summary_list->previous) {
    if(cJSON_AddStringToObject(item, "previous", paginated_location_summary_list->previous) == NULL) {
    goto fail; //String
    }
    }


    // paginated_location_summary_list->results
    if(paginated_location_summary_list->results) {
    ...
    if (paginated_location_summary_list->results) {
    list_ForEach(resultsListEntry, paginated_location_summary_list->results) {
    cJSON *itemLocal = location_summary_convertToJSON(resultsListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
...
paginated_location_summary_list_t *paginated_location_summary_list_parseFromJSON(cJSON *paginated_location_summary_listJSON){
    ...
    // paginated_location_summary_list->count
    cJSON *count = cJSON_GetObjectItemCaseSensitive(paginated_location_summary_listJSON, "count");
    ...
    // paginated_location_summary_list->next
    cJSON *next = cJSON_GetObjectItemCaseSensitive(paginated_location_summary_listJSON, "next");
    ...
    // paginated_location_summary_list->previous
    cJSON *previous = cJSON_GetObjectItemCaseSensitive(paginated_location_summary_listJSON, "previous");
    ...
    // paginated_location_summary_list->results
    cJSON *results = cJSON_GetObjectItemCaseSensitive(paginated_location_summary_listJSON, "results");
    ...
    cJSON_ArrayForEach(results_local_nonprimitive,results )
    {
        ...
        location_summary_t *resultsItem = location_summary_parseFromJSON(results_local_nonprimitive);
