...
static move_battle_style_summary_t *move_battle_style_summary_create_internal(
    char *name,
    char *url
    ) {
    move_battle_style_summary_t *move_battle_style_summary_local_var = malloc(sizeof(move_battle_style_summary_t));
...
void move_battle_style_summary_free(move_battle_style_summary_t *move_battle_style_summary) {
    if(NULL == move_battle_style_summary){
        return ;
    }
    if(move_battle_style_summary->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "move_battle_style_summary_free");
        return ;
    }
    listEntry_t *listEntry;
    if (move_battle_style_summary->name) {
        free(move_battle_style_summary->name);
        move_battle_style_summary->name = NULL;
    }
    if (move_battle_style_summary->url) {
        free(move_battle_style_summary->url);
        move_battle_style_summary->url = NULL;
    }
    free(move_battle_style_summary);
}
...
cJSON *move_battle_style_summary_convertToJSON(move_battle_style_summary_t *move_battle_style_summary) {
    ...
    if(cJSON_AddStringToObject(item, "name", move_battle_style_summary->name) == NULL) {
    goto fail; //String
    }


    // move_battle_style_summary->url
    if (!move_battle_style_summary->url) {
        goto fail;
    }
    if(cJSON_AddStringToObject(item, "url", move_battle_style_summary->url) == NULL) {
    goto fail; //String
    }
...
move_battle_style_summary_t *move_battle_style_summary_parseFromJSON(cJSON *move_battle_style_summaryJSON){
    ...
    // move_battle_style_summary->url
    cJSON *url = cJSON_GetObjectItemCaseSensitive(move_battle_style_summaryJSON, "url");
    if (cJSON_IsNull(url)) {
        url = NULL;
    }
    if (!url) {
