...
#include <string.h>
#include <stdio.h>
#include "encounter_condition_value_detail.h"
...
static encounter_condition_value_detail_t *encounter_condition_value_detail_create_internal(
    ...
    encounter_condition_value_detail_t *encounter_condition_value_detail_local_var = malloc(sizeof(encounter_condition_value_detail_t));
...
void encounter_condition_value_detail_free(encounter_condition_value_detail_t *encounter_condition_value_detail) {
    if(NULL == encounter_condition_value_detail){
        return ;
    }
    if(encounter_condition_value_detail->_library_owned != 1){
        fprintf(stderr, "WARNING: %s() does NOT free objects allocated by the user\n", "encounter_condition_value_detail_free");
        return ;
    }
    ...
    if (encounter_condition_value_detail->condition) {
        encounter_condition_summary_free(encounter_condition_value_detail->condition);
        encounter_condition_value_detail->condition = NULL;
    }
    if (encounter_condition_value_detail->names) {
        list_ForEach(listEntry, encounter_condition_value_detail->names) {
            encounter_condition_value_name_free(listEntry->data);
        }
        list_freeList(encounter_condition_value_detail->names);
        encounter_condition_value_detail->names = NULL;
    }
    free(encounter_condition_value_detail);
}
...
cJSON *encounter_condition_value_detail_convertToJSON(encounter_condition_value_detail_t *encounter_condition_value_detail) {
    ...
    // encounter_condition_value_detail->id
    ...
    if(cJSON_AddStringToObject(item, "name", encounter_condition_value_detail->name) == NULL) {
    goto fail; //String
    }


    // encounter_condition_value_detail->condition
    if (!encounter_condition_value_detail->condition) {
        goto fail;
    }
    cJSON *condition_local_JSON = encounter_condition_summary_convertToJSON(encounter_condition_value_detail->condition);
    ...
    // encounter_condition_value_detail->names
    if (!encounter_condition_value_detail->names) {
        goto fail;
    }
    ...
    if (encounter_condition_value_detail->names) {
    list_ForEach(namesListEntry, encounter_condition_value_detail->names) {
    cJSON *itemLocal = encounter_condition_value_name_convertToJSON(namesListEntry->data);
    if(itemLocal == NULL) {
    goto fail;
    }
...
encounter_condition_value_detail_t *encounter_condition_value_detail_parseFromJSON(cJSON *encounter_condition_value_detailJSON){
    ...
    // define the local list for encounter_condition_value_detail->names
    list_t *namesList = NULL;

    // encounter_condition_value_detail->id
    cJSON *id = cJSON_GetObjectItemCaseSensitive(encounter_condition_value_detailJSON, "id");
    if (cJSON_IsNull(id)) {
        id = NULL;
    }
    if (!id) {
    ...
    // encounter_condition_value_detail->condition
    cJSON *condition = cJSON_GetObjectItemCaseSensitive(encounter_condition_value_detailJSON, "condition");
    ...
    condition_local_nonprim = encounter_condition_summary_parseFromJSON(condition); //nonprimitive

    // encounter_condition_value_detail->names
    cJSON *names = cJSON_GetObjectItemCaseSensitive(encounter_condition_value_detailJSON, "names");
    ...
    cJSON_ArrayForEach(names_local_nonprimitive,names )
    {
        ...
        encounter_condition_value_name_t *namesItem = encounter_condition_value_name_parseFromJSON(names_local_nonprimitive);
