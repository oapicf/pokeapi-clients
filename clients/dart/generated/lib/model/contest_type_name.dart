...
class ContestTypeName {
  ...
  static ContestTypeName? fromJson(dynamic value) {
    if (value is Map) {
      ...
      assert(() {
        assert(json.containsKey(r'name'), 'Required key "ContestTypeName[name]" is missing from JSON.');
        assert(json[r'name'] != null, 'Required key "ContestTypeName[name]" has a null value in JSON.');
        assert(json.containsKey(r'color'), 'Required key "ContestTypeName[color]" is missing from JSON.');
        assert(json[r'color'] != null, 'Required key "ContestTypeName[color]" has a null value in JSON.');
        assert(json.containsKey(r'language'), 'Required key "ContestTypeName[language]" is missing from JSON.');
        assert(json[r'language'] != null, 'Required key "ContestTypeName[language]" has a null value in JSON.');
        return true;
      }());
