

#include "PaginatedPokedexSummaryList.h"

using namespace Tiny;

PaginatedPokedexSummaryList::PaginatedPokedexSummaryList()
{
	count = int(0);
	next = std::string();
	previous = std::string();
	results = std::list<PokedexSummary>();
}

PaginatedPokedexSummaryList::PaginatedPokedexSummaryList(std::string jsonString)
{
	this->fromJson(jsonString);
}

PaginatedPokedexSummaryList::~PaginatedPokedexSummaryList()
{

}

void
PaginatedPokedexSummaryList::fromJson(std::string jsonObj)
{
    bourne::json object = bourne::json::parse(jsonObj);

    const char *countKey = "count";

    if(object.has_key(countKey))
    {
        bourne::json value = object[countKey];



        jsonToValue(&count, value, "int");


    }

    const char *nextKey = "next";

    if(object.has_key(nextKey))
    {
        bourne::json value = object[nextKey];



        jsonToValue(&next, value, "std::string");


    }

    const char *previousKey = "previous";

    if(object.has_key(previousKey))
    {
        bourne::json value = object[previousKey];



        jsonToValue(&previous, value, "std::string");


    }

    const char *resultsKey = "results";

    if(object.has_key(resultsKey))
    {
        bourne::json value = object[resultsKey];


        std::list<PokedexSummary> results_list;
        PokedexSummary element;
        for(auto& var : value.array_range())
        {


            element.fromJson(var.dump());

            results_list.push_back(element);
        }
        results = results_list;


    }


}

bourne::json
PaginatedPokedexSummaryList::toJson()
{
    bourne::json object = bourne::json::object();





    object["count"] = getCount();






    object["next"] = getNext();






    object["previous"] = getPrevious();





    std::list<PokedexSummary> results_list = getResults();
    bourne::json results_arr = bourne::json::array();

    for(auto& var : results_list)
    {
        PokedexSummary obj = var;
        results_arr.append(obj.toJson());
    }
    object["results"] = results_arr;




    return object;

}

int
PaginatedPokedexSummaryList::getCount()
{
	return count;
}

void
PaginatedPokedexSummaryList::setCount(int count)
{
	this->count = count;
}

std::string
PaginatedPokedexSummaryList::getNext()
{
	return next;
}

void
PaginatedPokedexSummaryList::setNext(std::string next)
{
	this->next = next;
}

std::string
PaginatedPokedexSummaryList::getPrevious()
{
	return previous;
}

void
PaginatedPokedexSummaryList::setPrevious(std::string previous)
{
	this->previous = previous;
}

std::list<PokedexSummary>
PaginatedPokedexSummaryList::getResults()
{
	return results;
}

void
PaginatedPokedexSummaryList::setResults(std::list<PokedexSummary> results)
{
	this->results = results;
}



