

#include "PaginatedEvolutionTriggerSummaryList.h"

using namespace Tiny;

PaginatedEvolutionTriggerSummaryList::PaginatedEvolutionTriggerSummaryList()
{
	count = int(0);
	next = std::string();
	previous = std::string();
	results = std::list<EvolutionTriggerSummary>();
}

PaginatedEvolutionTriggerSummaryList::PaginatedEvolutionTriggerSummaryList(std::string jsonString)
{
	this->fromJson(jsonString);
}

PaginatedEvolutionTriggerSummaryList::~PaginatedEvolutionTriggerSummaryList()
{

}

void
PaginatedEvolutionTriggerSummaryList::fromJson(std::string jsonObj)
{
    bourne::json object = bourne::json::parse(jsonObj);

    const char *countKey = "count";

    if(object.has_key(countKey))
    {
        bourne::json value = object[countKey];



        jsonToValue(&count, value, "int");


    }

    const char *nextKey = "next";

    if(object.has_key(nextKey))
    {
        bourne::json value = object[nextKey];



        jsonToValue(&next, value, "std::string");


    }

    const char *previousKey = "previous";

    if(object.has_key(previousKey))
    {
        bourne::json value = object[previousKey];



        jsonToValue(&previous, value, "std::string");


    }

    const char *resultsKey = "results";

    if(object.has_key(resultsKey))
    {
        bourne::json value = object[resultsKey];


        std::list<EvolutionTriggerSummary> results_list;
        EvolutionTriggerSummary element;
        for(auto& var : value.array_range())
        {


            element.fromJson(var.dump());

            results_list.push_back(element);
        }
        results = results_list;


    }


}

bourne::json
PaginatedEvolutionTriggerSummaryList::toJson()
{
    bourne::json object = bourne::json::object();





    object["count"] = getCount();






    object["next"] = getNext();






    object["previous"] = getPrevious();





    std::list<EvolutionTriggerSummary> results_list = getResults();
    bourne::json results_arr = bourne::json::array();

    for(auto& var : results_list)
    {
        EvolutionTriggerSummary obj = var;
        results_arr.append(obj.toJson());
    }
    object["results"] = results_arr;




    return object;

}

int
PaginatedEvolutionTriggerSummaryList::getCount()
{
	return count;
}

void
PaginatedEvolutionTriggerSummaryList::setCount(int  count)
{
	this->count = count;
}

std::string
PaginatedEvolutionTriggerSummaryList::getNext()
{
	return next;
}

void
PaginatedEvolutionTriggerSummaryList::setNext(std::string  next)
{
	this->next = next;
}

std::string
PaginatedEvolutionTriggerSummaryList::getPrevious()
{
	return previous;
}

void
PaginatedEvolutionTriggerSummaryList::setPrevious(std::string  previous)
{
	this->previous = previous;
}

std::list<EvolutionTriggerSummary>
PaginatedEvolutionTriggerSummaryList::getResults()
{
	return results;
}

void
PaginatedEvolutionTriggerSummaryList::setResults(std::list <EvolutionTriggerSummary> results)
{
	this->results = results;
}



