

#include "PaginatedEncounterConditionSummaryList.h"

using namespace Tiny;

PaginatedEncounterConditionSummaryList::PaginatedEncounterConditionSummaryList()
{
	count = int(0);
	next = std::string();
	previous = std::string();
	results = std::list<EncounterConditionSummary>();
}

PaginatedEncounterConditionSummaryList::PaginatedEncounterConditionSummaryList(std::string jsonString)
{
	this->fromJson(jsonString);
}

PaginatedEncounterConditionSummaryList::~PaginatedEncounterConditionSummaryList()
{

}

void
PaginatedEncounterConditionSummaryList::fromJson(std::string jsonObj)
{
    bourne::json object = bourne::json::parse(jsonObj);

    const char *countKey = "count";

    if(object.has_key(countKey))
    {
        bourne::json value = object[countKey];



        jsonToValue(&count, value, "int");


    }

    const char *nextKey = "next";

    if(object.has_key(nextKey))
    {
        bourne::json value = object[nextKey];



        jsonToValue(&next, value, "std::string");


    }

    const char *previousKey = "previous";

    if(object.has_key(previousKey))
    {
        bourne::json value = object[previousKey];



        jsonToValue(&previous, value, "std::string");


    }

    const char *resultsKey = "results";

    if(object.has_key(resultsKey))
    {
        bourne::json value = object[resultsKey];


        std::list<EncounterConditionSummary> results_list;
        EncounterConditionSummary element;
        for(auto& var : value.array_range())
        {


            element.fromJson(var.dump());

            results_list.push_back(element);
        }
        results = results_list;


    }


}

bourne::json
PaginatedEncounterConditionSummaryList::toJson()
{
    bourne::json object = bourne::json::object();





    object["count"] = getCount();






    object["next"] = getNext();






    object["previous"] = getPrevious();





    std::list<EncounterConditionSummary> results_list = getResults();
    bourne::json results_arr = bourne::json::array();

    for(auto& var : results_list)
    {
        EncounterConditionSummary obj = var;
        results_arr.append(obj.toJson());
    }
    object["results"] = results_arr;




    return object;

}

int
PaginatedEncounterConditionSummaryList::getCount()
{
	return count;
}

void
PaginatedEncounterConditionSummaryList::setCount(int count)
{
	this->count = count;
}

std::string
PaginatedEncounterConditionSummaryList::getNext()
{
	return next;
}

void
PaginatedEncounterConditionSummaryList::setNext(std::string next)
{
	this->next = next;
}

std::string
PaginatedEncounterConditionSummaryList::getPrevious()
{
	return previous;
}

void
PaginatedEncounterConditionSummaryList::setPrevious(std::string previous)
{
	this->previous = previous;
}

std::list<EncounterConditionSummary>
PaginatedEncounterConditionSummaryList::getResults()
{
	return results;
}

void
PaginatedEncounterConditionSummaryList::setResults(std::list<EncounterConditionSummary> results)
{
	this->results = results;
}



