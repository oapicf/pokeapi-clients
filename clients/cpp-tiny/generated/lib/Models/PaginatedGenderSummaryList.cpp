

#include "PaginatedGenderSummaryList.h"

using namespace Tiny;

PaginatedGenderSummaryList::PaginatedGenderSummaryList()
{
	count = int(0);
	next = std::string();
	previous = std::string();
	results = std::list<GenderSummary>();
}

PaginatedGenderSummaryList::PaginatedGenderSummaryList(std::string jsonString)
{
	this->fromJson(jsonString);
}

PaginatedGenderSummaryList::~PaginatedGenderSummaryList()
{

}

void
PaginatedGenderSummaryList::fromJson(std::string jsonObj)
{
    bourne::json object = bourne::json::parse(jsonObj);

    const char *countKey = "count";

    if(object.has_key(countKey))
    {
        bourne::json value = object[countKey];



        jsonToValue(&count, value, "int");


    }

    const char *nextKey = "next";

    if(object.has_key(nextKey))
    {
        bourne::json value = object[nextKey];



        jsonToValue(&next, value, "std::string");


    }

    const char *previousKey = "previous";

    if(object.has_key(previousKey))
    {
        bourne::json value = object[previousKey];



        jsonToValue(&previous, value, "std::string");


    }

    const char *resultsKey = "results";

    if(object.has_key(resultsKey))
    {
        bourne::json value = object[resultsKey];


        std::list<GenderSummary> results_list;
        GenderSummary element;
        for(auto& var : value.array_range())
        {


            element.fromJson(var.dump());

            results_list.push_back(element);
        }
        results = results_list;


    }


}

bourne::json
PaginatedGenderSummaryList::toJson()
{
    bourne::json object = bourne::json::object();





    object["count"] = getCount();






    object["next"] = getNext();






    object["previous"] = getPrevious();





    std::list<GenderSummary> results_list = getResults();
    bourne::json results_arr = bourne::json::array();

    for(auto& var : results_list)
    {
        GenderSummary obj = var;
        results_arr.append(obj.toJson());
    }
    object["results"] = results_arr;




    return object;

}

int
PaginatedGenderSummaryList::getCount()
{
	return count;
}

void
PaginatedGenderSummaryList::setCount(int  count)
{
	this->count = count;
}

std::string
PaginatedGenderSummaryList::getNext()
{
	return next;
}

void
PaginatedGenderSummaryList::setNext(std::string  next)
{
	this->next = next;
}

std::string
PaginatedGenderSummaryList::getPrevious()
{
	return previous;
}

void
PaginatedGenderSummaryList::setPrevious(std::string  previous)
{
	this->previous = previous;
}

std::list<GenderSummary>
PaginatedGenderSummaryList::getResults()
{
	return results;
}

void
PaginatedGenderSummaryList::setResults(std::list <GenderSummary> results)
{
	this->results = results;
}



