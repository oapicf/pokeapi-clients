...
-- | display the config
instance P.Show PokéConfig where
  show c =
    T.printf
...
-- | constructs a default PokéConfig
--
-- configHost:
--
-- @https://pokeapi.co@
...
-- @"poké/0.1.0.0"@
--
newConfig :: IO PokéConfig
newConfig = do
    logCxt <- initLogContext
    return $ PokéConfig
        { configHost = "https://pokeapi.co"
        , configUserAgent = "poké/0.1.0.0"
...
addAuthMethod :: AuthMethod auth => PokéConfig -> auth -> PokéConfig
addAuthMethod config@PokéConfig {configAuthMethods = as} a =
  config { configAuthMethods = AnyAuthMethod a : as}

-- | updates the config to use stdout logging
withStdoutLogging :: PokéConfig -> IO PokéConfig
...
withStderrLogging :: PokéConfig -> IO PokéConfig
...
withNoLogging :: PokéConfig -> PokéConfig
...
data PokéRequest req contentType res accept = PokéRequest
  { rMethod  :: NH.Method   -- ^ Method of PokéRequest
  , rUrlPath :: [BCL.ByteString] -- ^ Endpoint of PokéRequest
  , rParams   :: Params -- ^ params of PokéRequest
  , rAuthTypes :: [P.TypeRep] -- ^ types of auth methods
  }
  deriving (P.Show)

-- | 'rMethod' Lens
rMethodL :: Lens_' (PokéRequest req contentType res accept) NH.Method
rMethodL f PokéRequest{..} = (\rMethod -> PokéRequest { rMethod, ..} ) <$> f rMethod
{-# INLINE rMethodL #-}

-- | 'rUrlPath' Lens
rUrlPathL :: Lens_' (PokéRequest req contentType res accept) [BCL.ByteString]
rUrlPathL f PokéRequest{..} = (\rUrlPath -> PokéRequest { rUrlPath, ..} ) <$> f rUrlPath
{-# INLINE rUrlPathL #-}

-- | 'rParams' Lens
rParamsL :: Lens_' (PokéRequest req contentType res accept) Params
rParamsL f PokéRequest{..} = (\rParams -> PokéRequest { rParams, ..} ) <$> f rParams
{-# INLINE rParamsL #-}

-- | 'rParams' Lens
rAuthTypesL :: Lens_' (PokéRequest req contentType res accept) [P.TypeRep]
rAuthTypesL f PokéRequest{..} = (\rAuthTypes -> PokéRequest { rAuthTypes, ..} ) <$> f rAuthTypes
{-# INLINE rAuthTypesL #-}
...
class HasBodyParam req param where
  setBodyParam :: forall contentType res accept. (Consumes req contentType, MimeRender contentType param) => PokéRequest req contentType res accept -> param -> PokéRequest req contentType res accept
  setBodyParam req xs =
...
class HasOptionalParam req param where
  ...
  applyOptionalParam :: PokéRequest req contentType res accept -> param -> PokéRequest req contentType res accept
  applyOptionalParam = (-&-)
  {-# INLINE applyOptionalParam #-}

  -- | infix operator \/ alias for 'addOptionalParam'
  (-&-) :: PokéRequest req contentType res accept -> param -> PokéRequest req contentType res accept
  (-&-) = applyOptionalParam
  {-# INLINE (-&-) #-}
...
_mkRequest :: NH.Method -- ^ Method
          ...
          -> PokéRequest req contentType res accept -- ^ req: Request Type, res: Response Type
_mkRequest m u = PokéRequest m u _mkParams []

_mkParams :: Params
_mkParams = Params [] [] ParamBodyNone

setHeader ::
     PokéRequest req contentType res accept
  -> [NH.Header]
  -> PokéRequest req contentType res accept
...
removeHeader :: PokéRequest req contentType res accept -> [NH.HeaderName] -> PokéRequest req contentType res accept
...
_setContentTypeHeader :: forall req contentType res accept. MimeType contentType => PokéRequest req contentType res accept -> PokéRequest req contentType res accept
_setContentTypeHeader req =
    case mimeType (P.Proxy :: P.Proxy contentType) of
        Just m -> req `setHeader` [("content-type", BC.pack $ P.show m)]
        Nothing -> req `removeHeader` ["content-type"]

_setAcceptHeader :: forall req contentType res accept. MimeType accept => PokéRequest req contentType res accept -> PokéRequest req contentType res accept
...
addForm :: PokéRequest req contentType res accept -> WH.Form -> PokéRequest req contentType res accept
addForm req newform =
...
_addMultiFormPart :: PokéRequest req contentType res accept -> NH.Part -> PokéRequest req contentType res accept
...
_setBodyBS :: PokéRequest req contentType res accept -> B.ByteString -> PokéRequest req contentType res accept
_setBodyBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyB body)

_setBodyLBS :: PokéRequest req contentType res accept -> BL.ByteString -> PokéRequest req contentType res accept
_setBodyLBS req body =
    req & L.set (rParamsL . paramsBodyL) (ParamBodyBL body)

_hasAuthType :: AuthMethod authMethod => PokéRequest req contentType res accept -> P.Proxy authMethod -> PokéRequest req contentType res accept
...
class P.Typeable a =>
  ...
  applyAuthMethod
    ...
    -> a
    -> PokéRequest req contentType res accept
    -> IO (PokéRequest req contentType res accept)
...
_applyAuthMethods
  ...
  -> PokéConfig
  -> IO (PokéRequest req contentType res accept)
_applyAuthMethods req config@(PokéConfig {configAuthMethods = as}) =
  foldlM go req as
  where
