...
class MachinesApiVertxProxyHandler(private val vertx: Vertx, private val service: MachinesApi, topLevel: Boolean, private val timeoutSeconds: Long) : ProxyHandler() {
            ...
            when (action) {
                ...
                "machineList" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.machineList(limit,offset,q,context)
                ...
                "machineRetrieve" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.machineRetrieve(id,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
