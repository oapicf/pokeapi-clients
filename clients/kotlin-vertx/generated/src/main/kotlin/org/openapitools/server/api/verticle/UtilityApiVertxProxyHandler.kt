...
class UtilityApiVertxProxyHandler(private val vertx: Vertx, private val service: UtilityApi, topLevel: Boolean, private val timeoutSeconds: Long) : ProxyHandler() {
            ...
            when (action) {
                ...
                "languageList" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.languageList(limit,offset,q,context)
                ...
                "languageRetrieve" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.languageRetrieve(id,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
