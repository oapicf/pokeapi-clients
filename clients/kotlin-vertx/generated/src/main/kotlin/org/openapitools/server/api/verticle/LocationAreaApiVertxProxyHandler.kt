...
class LocationAreaApiVertxProxyHandler(private val vertx: Vertx, private val service: LocationAreaApi, topLevel: Boolean, private val timeoutSeconds: Long) : ProxyHandler() {
        ...
        try {
            ...
            when (action) {
                ...
                "locationAreaRead" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.locationAreaRead(id,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
