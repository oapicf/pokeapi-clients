...
class BerriesApiVertxProxyHandler(private val vertx: Vertx, private val service: BerriesApi, topLevel: Boolean, private val timeoutSeconds: Long) : ProxyHandler() {
            ...
            when (action) {
                ...
                "berryFirmnessList" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.berryFirmnessList(limit,offset,q,context)
                ...
                "berryFirmnessRetrieve" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.berryFirmnessRetrieve(id,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
                ...
                "berryFlavorList" -> {
                    ...
                    val q = ApiHandlerUtils.searchStringInJson(params,"q")
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.berryFlavorList(limit,offset,q,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
                ...
                "berryFlavorRetrieve" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.berryFlavorRetrieve(id,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
                ...
                "berryList" -> {
                    ...
                    val q = ApiHandlerUtils.searchStringInJson(params,"q")
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.berryList(limit,offset,q,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
                ...
                "berryRetrieve" -> {
                    ...
                    GlobalScope.launch(vertx.dispatcher()){
                        val result = service.berryRetrieve(id,context)
                        val payload = JsonObject(Json.encode(result.payload)).toBuffer()
                        val res = OperationResponse(result.statusCode,result.statusMessage,payload,result.headers)
                        msg.reply(res.toJson())
                    }.invokeOnCompletion{
