...
async function example() {
  ...
  const api = new SuperContestEffectApi();
...
import type { SuperContestEffectReadRequest } from '';
...
async function example() {
  ...
  const body = {
    // number
    id: 56,
  } satisfies SuperContestEffectReadRequest;

  try {
    const data = await api.superContestEffectRead(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
