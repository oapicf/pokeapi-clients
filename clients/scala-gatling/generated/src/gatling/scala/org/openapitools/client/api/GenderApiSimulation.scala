...
class GenderApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val genderReadPerSecond = config.getDouble("performance.operationsPerSecond.genderRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val gender_listQUERYFeeder = csv(userDataDirectory + File.separator + "genderList-queryParams.csv").random
    val gender_readPATHFeeder = csv(userDataDirectory + File.separator + "genderRead-pathParams.csv").random
    ...
    val scngenderRead = scenario("genderReadSimulation")
        .feed(gender_readPATHFeeder)
        .exec(http("genderRead")
        .httpRequest("GET","/api/v2/gender/${id}/")
)

    // Run scngenderRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scngenderRead.inject(
        rampUsersPerSec(1) to(genderReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(genderReadPerSecond) during(durationSeconds),
        rampUsersPerSec(genderReadPerSecond) to(1) during(rampDownSeconds)
    )
