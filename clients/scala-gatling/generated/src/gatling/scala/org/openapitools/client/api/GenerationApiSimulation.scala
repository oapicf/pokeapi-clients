...
class GenerationApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val generationReadPerSecond = config.getDouble("performance.operationsPerSecond.generationRead") * rateMultiplier * instanceMultiplier
    ...
    val generation_readPATHFeeder = csv(userDataDirectory + File.separator + "generationRead-pathParams.csv").random
    ...
    val scngenerationRead = scenario("generationReadSimulation")
        .feed(generation_readPATHFeeder)
        .exec(http("generationRead")
        .httpRequest("GET","/api/v2/generation/${id}/")
)

    // Run scngenerationRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scngenerationRead.inject(
        rampUsersPerSec(1) to(generationReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(generationReadPerSecond) during(durationSeconds),
        rampUsersPerSec(generationReadPerSecond) to(1) during(rampDownSeconds)
    )
