...
// Setup http protocol configuration
    ...
    val versionReadPerSecond = config.getDouble("performance.operationsPerSecond.versionRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val version_listQUERYFeeder = csv(userDataDirectory + File.separator + "versionList-queryParams.csv").random
    val version_readPATHFeeder = csv(userDataDirectory + File.separator + "versionRead-pathParams.csv").random
    ...
    val scnversionRead = scenario("versionReadSimulation")
        .feed(version_readPATHFeeder)
        .exec(http("versionRead")
        .httpRequest("GET","/api/v2/version/${id}/")
)

    // Run scnversionRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnversionRead.inject(
        rampUsersPerSec(1) to(versionReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(versionReadPerSecond) during(durationSeconds),
        rampUsersPerSec(versionReadPerSecond) to(1) during(rampDownSeconds)
    )
