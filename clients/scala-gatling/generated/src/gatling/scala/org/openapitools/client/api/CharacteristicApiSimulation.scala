...
class CharacteristicApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val characteristicReadPerSecond = config.getDouble("performance.operationsPerSecond.characteristicRead") * rateMultiplier * instanceMultiplier
    ...
    val characteristic_readPATHFeeder = csv(userDataDirectory + File.separator + "characteristicRead-pathParams.csv").random
    ...
    val scncharacteristicRead = scenario("characteristicReadSimulation")
        .feed(characteristic_readPATHFeeder)
        .exec(http("characteristicRead")
        .httpRequest("GET","/api/v2/characteristic/${id}/")
)

    // Run scncharacteristicRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scncharacteristicRead.inject(
        rampUsersPerSec(1) to(characteristicReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(characteristicReadPerSecond) during(durationSeconds),
        rampUsersPerSec(characteristicReadPerSecond) to(1) during(rampDownSeconds)
    )
