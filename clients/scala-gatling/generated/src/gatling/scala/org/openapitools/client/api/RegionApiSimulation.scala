...
class RegionApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val regionReadPerSecond = config.getDouble("performance.operationsPerSecond.regionRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val region_listQUERYFeeder = csv(userDataDirectory + File.separator + "regionList-queryParams.csv").random
    val region_readPATHFeeder = csv(userDataDirectory + File.separator + "regionRead-pathParams.csv").random
    ...
    val scnregionRead = scenario("regionReadSimulation")
        .feed(region_readPATHFeeder)
        .exec(http("regionRead")
        .httpRequest("GET","/api/v2/region/${id}/")
)

    // Run scnregionRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnregionRead.inject(
        rampUsersPerSec(1) to(regionReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(regionReadPerSecond) during(durationSeconds),
        rampUsersPerSec(regionReadPerSecond) to(1) during(rampDownSeconds)
    )
