...
class ItemApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val itemReadPerSecond = config.getDouble("performance.operationsPerSecond.itemRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val item_listQUERYFeeder = csv(userDataDirectory + File.separator + "itemList-queryParams.csv").random
    val item_readPATHFeeder = csv(userDataDirectory + File.separator + "itemRead-pathParams.csv").random
    ...
    val scnitemRead = scenario("itemReadSimulation")
        .feed(item_readPATHFeeder)
        .exec(http("itemRead")
        .httpRequest("GET","/api/v2/item/${id}/")
)

    // Run scnitemRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnitemRead.inject(
        rampUsersPerSec(1) to(itemReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(itemReadPerSecond) during(durationSeconds),
        rampUsersPerSec(itemReadPerSecond) to(1) during(rampDownSeconds)
    )
