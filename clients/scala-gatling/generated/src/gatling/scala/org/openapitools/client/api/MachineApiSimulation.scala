...
class MachineApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val machineReadPerSecond = config.getDouble("performance.operationsPerSecond.machineRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val machine_listQUERYFeeder = csv(userDataDirectory + File.separator + "machineList-queryParams.csv").random
    val machine_readPATHFeeder = csv(userDataDirectory + File.separator + "machineRead-pathParams.csv").random
    ...
    val scnmachineRead = scenario("machineReadSimulation")
        .feed(machine_readPATHFeeder)
        .exec(http("machineRead")
        .httpRequest("GET","/api/v2/machine/${id}/")
)

    // Run scnmachineRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnmachineRead.inject(
        rampUsersPerSec(1) to(machineReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(machineReadPerSecond) during(durationSeconds),
        rampUsersPerSec(machineReadPerSecond) to(1) during(rampDownSeconds)
    )
