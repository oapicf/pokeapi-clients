...
class StatApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val statReadPerSecond = config.getDouble("performance.operationsPerSecond.statRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val stat_listQUERYFeeder = csv(userDataDirectory + File.separator + "statList-queryParams.csv").random
    val stat_readPATHFeeder = csv(userDataDirectory + File.separator + "statRead-pathParams.csv").random
    ...
    val scnstatRead = scenario("statReadSimulation")
        .feed(stat_readPATHFeeder)
        .exec(http("statRead")
        .httpRequest("GET","/api/v2/stat/${id}/")
)

    // Run scnstatRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnstatRead.inject(
        rampUsersPerSec(1) to(statReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(statReadPerSecond) during(durationSeconds),
        rampUsersPerSec(statReadPerSecond) to(1) during(rampDownSeconds)
    )
