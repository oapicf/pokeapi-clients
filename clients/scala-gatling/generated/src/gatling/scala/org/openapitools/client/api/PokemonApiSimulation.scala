...
// Setup http protocol configuration
    ...
    val pokemonReadPerSecond = config.getDouble("performance.operationsPerSecond.pokemonRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val pokemon_listQUERYFeeder = csv(userDataDirectory + File.separator + "pokemonList-queryParams.csv").random
    val pokemon_readPATHFeeder = csv(userDataDirectory + File.separator + "pokemonRead-pathParams.csv").random
    ...
    val scnpokemonRead = scenario("pokemonReadSimulation")
        .feed(pokemon_readPATHFeeder)
        .exec(http("pokemonRead")
        .httpRequest("GET","/api/v2/pokemon/${id}/")
)

    // Run scnpokemonRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnpokemonRead.inject(
        rampUsersPerSec(1) to(pokemonReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(pokemonReadPerSecond) during(durationSeconds),
        rampUsersPerSec(pokemonReadPerSecond) to(1) during(rampDownSeconds)
    )
