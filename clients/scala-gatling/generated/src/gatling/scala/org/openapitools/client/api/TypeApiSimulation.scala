...
class TypeApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val typeReadPerSecond = config.getDouble("performance.operationsPerSecond.typeRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val type_listQUERYFeeder = csv(userDataDirectory + File.separator + "typeList-queryParams.csv").random
    val type_readPATHFeeder = csv(userDataDirectory + File.separator + "typeRead-pathParams.csv").random
    ...
    val scntypeRead = scenario("typeReadSimulation")
        .feed(type_readPATHFeeder)
        .exec(http("typeRead")
        .httpRequest("GET","/api/v2/type/${id}/")
)

    // Run scntypeRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scntypeRead.inject(
        rampUsersPerSec(1) to(typeReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(typeReadPerSecond) during(durationSeconds),
        rampUsersPerSec(typeReadPerSecond) to(1) during(rampDownSeconds)
    )
