...
class MoveApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val moveReadPerSecond = config.getDouble("performance.operationsPerSecond.moveRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val move_listQUERYFeeder = csv(userDataDirectory + File.separator + "moveList-queryParams.csv").random
    val move_readPATHFeeder = csv(userDataDirectory + File.separator + "moveRead-pathParams.csv").random
    ...
    val scnmoveRead = scenario("moveReadSimulation")
        .feed(move_readPATHFeeder)
        .exec(http("moveRead")
        .httpRequest("GET","/api/v2/move/${id}/")
)

    // Run scnmoveRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnmoveRead.inject(
        rampUsersPerSec(1) to(moveReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(moveReadPerSecond) during(durationSeconds),
        rampUsersPerSec(moveReadPerSecond) to(1) during(rampDownSeconds)
    )
