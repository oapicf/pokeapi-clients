...
class NatureApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val natureReadPerSecond = config.getDouble("performance.operationsPerSecond.natureRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val nature_listQUERYFeeder = csv(userDataDirectory + File.separator + "natureList-queryParams.csv").random
    val nature_readPATHFeeder = csv(userDataDirectory + File.separator + "natureRead-pathParams.csv").random
    ...
    val scnnatureRead = scenario("natureReadSimulation")
        .feed(nature_readPATHFeeder)
        .exec(http("natureRead")
        .httpRequest("GET","/api/v2/nature/${id}/")
)

    // Run scnnatureRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnnatureRead.inject(
        rampUsersPerSec(1) to(natureReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(natureReadPerSecond) during(durationSeconds),
        rampUsersPerSec(natureReadPerSecond) to(1) during(rampDownSeconds)
    )
