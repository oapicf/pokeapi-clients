...
class BerryApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val berryReadPerSecond = config.getDouble("performance.operationsPerSecond.berryRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val berry_listQUERYFeeder = csv(userDataDirectory + File.separator + "berryList-queryParams.csv").random
    val berry_readPATHFeeder = csv(userDataDirectory + File.separator + "berryRead-pathParams.csv").random
    ...
    val scnberryRead = scenario("berryReadSimulation")
        .feed(berry_readPATHFeeder)
        .exec(http("berryRead")
        .httpRequest("GET","/api/v2/berry/${id}/")
)

    // Run scnberryRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnberryRead.inject(
        rampUsersPerSec(1) to(berryReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(berryReadPerSecond) during(durationSeconds),
        rampUsersPerSec(berryReadPerSecond) to(1) during(rampDownSeconds)
    )
