...
class AbilityApiSimulation extends Simulation {
...
// Setup http protocol configuration
    ...
    val abilityReadPerSecond = config.getDouble("performance.operationsPerSecond.abilityRead") * rateMultiplier * instanceMultiplier

    val scenarioBuilders: mutable.MutableList[PopulationBuilder] = new mutable.MutableList[PopulationBuilder]()

    // Set up CSV feeders
    val ability_listQUERYFeeder = csv(userDataDirectory + File.separator + "abilityList-queryParams.csv").random
    val ability_readPATHFeeder = csv(userDataDirectory + File.separator + "abilityRead-pathParams.csv").random
    ...
    val scnabilityRead = scenario("abilityReadSimulation")
        .feed(ability_readPATHFeeder)
        .exec(http("abilityRead")
        .httpRequest("GET","/api/v2/ability/${id}/")
)

    // Run scnabilityRead with warm up and reach a constant rate for entire duration
    scenarioBuilders += scnabilityRead.inject(
        rampUsersPerSec(1) to(abilityReadPerSecond) during(rampUpSeconds),
        constantUsersPerSec(abilityReadPerSecond) during(durationSeconds),
        rampUsersPerSec(abilityReadPerSecond) to(1) during(rampDownSeconds)
    )
