...
const { status, data } = await apiInstance.pokemonHabitatList(
    limit,
    offset
);
```
...
const { status, data } = await apiInstance.pokemonHabitatRead(
    id
);
```
