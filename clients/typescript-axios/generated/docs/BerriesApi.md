...
const { status, data } = await apiInstance.berryFirmnessRetrieve(
    id
);
```
...
const { status, data } = await apiInstance.berryFlavorRetrieve(
    id
);
```
...
const { status, data } = await apiInstance.berryRetrieve(
    id
);
```
