...
const { status, data } = await apiInstance.moveDamageClassList(
    limit,
    offset
);
```
...
const { status, data } = await apiInstance.moveDamageClassRead(
    id
);
```
